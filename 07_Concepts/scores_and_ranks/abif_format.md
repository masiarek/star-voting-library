...
## Comparing like with like — ABIF ≈ our `ballots:` block
...
- **Everything wrapped around the ballots differs in kind.** Our [YAML test case](../about_this_repo/04_YAML_authoring_template.md) also carries the **method** (`voting_method`), **seats** (`num_winners`), reporting **`options:`**, human context (`scenario_description`), and — the big one — an **answer key** (`expected_winners`) that CI enforces. ABIF's metadata is a lighter `{ … }` block (title, candidate list); it has no notion of "the winner this file must produce," because it describes *ballots*, it doesn't assert a *test*.
