...
The examples above are all **single-winner**, where the summable artifact is tiny — but the two shapes are *not* the same size, and the distinction is the whole point of the "order" language. One score or approval total per candidate is a **vector**: `O(candidates)` numbers, **first-order** summable. A **pairwise matrix** is a grid of every candidate against every other: `O(candidates²)` numbers, **second-order**. Both are trivially publishable and both are independent of how many people voted — that's what makes them summable — but a Condorcet method's artifact is genuinely the bigger one. (Formal definition: the [summability criterion](https://electowiki.org/wiki/Summability_criterion).)

**Careful — order is not the same idea as [what statistic a method reads](../condorcet/condorcet_reading_list.md).** Fishburn's C1/C2/C3 tiers classify *which* pairwise information a rule needs; summability order measures *how big* the published artifact is. They come apart at exactly one famous point: **plurality is C3** — its winner is not a function of the pairwise matrix at all — yet it is the cheapest first-order summable method there is. So the tiers are not a summability ladder and must never be read as one.
...
There's an asymmetry that matters for this repo's [PR comparison](../../../method_comparisons/stv_vs_star_pr/README.md): the **STAR-PR family** (Allocated Score, Sequentially Spent Score, RRV) *is* compatible with seat capping, so it can be made summable to a small order — but **STV is not**, because it eliminates never-elected candidates to free up their votes for transfer, and no fixed precinct total can anticipate which. So STV's non-summability runs deeper than IRV's: even the seat-capping workaround can't rescue it, whereas the STAR-PR methods it's usually compared against can be made precinct-verifiable at 3-seat districts.
...
*This is a **topic hub** (cross-method index). The authoritative write-ups live in the per-method folders linked above; this page just gathers the summability angle in one place for folks browsing by topic. See [the topics index](../README.md) for the other topic hubs.*
