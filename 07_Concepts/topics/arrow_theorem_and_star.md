...
- **Gibbard's theorem still applies.** Gibbard (1973), extended by Gibbard (1978) to cardinal and game-form methods, proves that *every* non-dictatorial deterministic method with 3+ outcomes is **manipulable** — some situation always exists where a voter gains by voting insincerely. STAR is no exception ([strategic voting](strategic_voting.md); the worked [5-1-0 challenge](../../method_comparisons/star_5_1_0_challenge/README.md)). So "STAR escapes **Arrow**" is true; "STAR escapes **every** impossibility theorem" is not. The one it doesn't escape is Gibbard.
- **IIA isn't free either.** The pure score *sum* is IIA-clean — adding a hopeless candidate doesn't change anyone else's total (unless voters renormalize). But STAR's *runoff* is IIA-sensitive inside a [Condorcet cycle](../../05_Ranked_Robin/01_Learn/cycle_resolution.md): a candidate who can't win can still change *which two* reach the runoff, and flip the result — worked, on sincere ballots, in [the cycle spoiler (BV2212)](../../01_STAR/03_Criteria/iia_cycle_spoiler/README.md).
