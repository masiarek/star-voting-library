...
- **IIA isn't free either.** The pure score *sum* is IIA-clean — adding a hopeless candidate doesn't change anyone else's total (unless voters renormalize). But STAR's *runoff* is IIA-sensitive inside a [Condorcet cycle](../../05_Ranked_Robin/concepts/cycle_resolution.md): a candidate who can't win can still change *which two* reach the runoff, and flip the result — worked, on sincere ballots, in [the cycle spoiler (BV2212)](../../01_STAR/iia_cycle_spoiler/).
