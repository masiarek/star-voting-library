...
> **One trap worth recording, because it nearly became a published finding.** The first version of this sweep drew its spatial electorates with `generate_profile(probmodel="euclidean", seed=…)` and reported **0.00% cycles in every single spatial cell** — a tidy, quotable, completely false result. That path collapses when given a seed: 2,000 different seeds returned 2,000 copies of one degenerate profile in which every voter submits the identical ranking. The defect is not in `pref_voting` but one layer down in **prefsampling 0.1.24**, which it calls — `GAUSSIAN_BALL` returns a single point repeated once per voter, and the candidate draw collapses onto that same point, so every distance is zero, every ranking ties, and the count falls back to index order. Filed upstream as [prefsampling#6](https://github.com/COMSOC-Community/prefsampling/issues/6), which also covers a second defect found alongside it: candidates are drawn from the same seeded stream as the voters and land exactly on the first voters — on four of the six spaces at unequal counts, and on **all six** — including `uniform_ball`, the one `pref_voting` uses by default — whenever `num_voters == num_candidates`. Unseeded calls are fine — so seeding *for reproducibility* is what breaks it. The sweep now draws positions from a numpy generator it owns. A simulation that returns a suspiciously clean zero deserves a check that the sampler is sampling. What the six space names mean, and the same trap in one paragraph: [the six Euclidean spaces](../euclidean_spaces.md). The full mechanical account — why each defect happens, why the two Gaussian spaces behave differently, and a probe that measures every claim — is in the companion QA repo: [prefsampling's seeded Euclidean samplers](https://masiarek.github.io/bettervoting-qa/analysis/prefsampling-seeding/index.html).
...
- [What a method reads (C1 / C2 / C3)](../what_a_method_reads.md) · [Election simulation models](../election_simulation_models.md) · [The spatial model](../spatial_voting_model.md) · [The six Euclidean spaces](../euclidean_spaces.md)
