...
The tempting reading is that this is the cardinal/ordinal line, but it isn't — Approval is a rated ballot and Ranked Robin is purely ordinal, and they land on opposite sides. The line that actually divides them is procedural: **every method here that finishes with a head-to-head elects banana, and the two that never take a majority vote elect orange.** Reading intensity is what *lets* a method reach the utilitarian answer — ranked methods can only see order, [**preference**, not **support**](../scores_and_ranks/preference_vs_support.md), so they can only ever chase the majoritarian ideal — but reading it is not enough to *keep* that answer. STAR reads the intensity, names orange, and then overrules itself. The value on offer isn't the "right" answer — it's that **both answers are on screen and the report says which one it acted on**. (Full write-up, and all three races: [Majoritarian vs. utilitarian](../../method_comparisons/majoritarian_vs_utilitarian/README.md).)
...
**The simulator is public, so the assumptions behind any VSE number are checkable rather than arguable** — the electorate it was run on, what its "strategic" voters actually do, and how good their polls are all live in one readable codebase: [vse-sim, read from source](vse_sim.md).
