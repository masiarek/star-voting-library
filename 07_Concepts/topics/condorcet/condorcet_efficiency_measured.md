...
**Ranked Robin is the control, not a result.** [Copeland](../../../05_Ranked_Robin/01_Learn/ranked_robin.md) is Condorcet-efficient by construction, so its column *must* read exactly 100.0%. It is printed precisely so that a reader can check the harness: any cell below 100.0% would mean the pairwise code and the method code disagree, and every other number in the run would be worthless.
...
**3. In structured electorates the ordering reverses, and the reversal grows with the field.** In 1-D spatial — the classic left-right spectrum, and precisely where [center squeeze](../center_squeeze/README.md) lives — IRV falls from 89.1% at three candidates to **47.0%** at seven, while STAR holds 74–79%. At seven candidates in 1-D, IRV elects the head-to-head winner **less than half the time**. That is the center-squeeze mechanism showing up as a statistic instead of an anecdote, and it is a much stronger result for STAR than the impartial-culture number is against it.
...
- **Ballot resolution is not held constant across methods** — ranked methods get full-resolution preferences, score methods get six rungs. Realistic (a 0–5 ballot genuinely cannot rank seven candidates), but it means part of the STAR-vs-RR gap at large fields measures **ballot expressiveness rather than tabulation rule**. This one is now measured rather than merely flagged, and it was understated: **[What the ballot can and cannot say](../../scores_and_ranks/ballot_expressiveness_measured.md)** re-runs the harness with the ballot and the rule varied independently, and finds that at seven candidates in `spatial1d` roughly **80% of STAR's gap against the control is the paper, not the count** — STAR's rule at full resolution scores 98–100%. It also finds the correction does *not* favour ranked ballots in general: capped at five names the way real jurisdictions cap them, the ranked ballot falls far below the 0–5 score ballot, and RCV-IRV cannot spend the extra resolution at all.
...
- **Tie-breaks are lowest-column-index throughout** — on an exact tie, the candidate in the leftmost ballot column wins. Arbitrary, but identically arbitrary for every method, which is what keeps the comparison fair; and a tie-break has to be arbitrary *somewhere* ([ties are forced](../ties/ties_are_forced.md)). **STAR is the one exception**, because it uses the real engine's [tie-break rungs](../../../01_STAR/01_Learn/Tie_Breaking_STAR/tie_breaking.md) — head-to-head among the tied, then five-star counts, then the lot, which is that same lowest-column-index order at the bottom. That is not a second implementation of STAR: this script imports the model from [`star_vs_rr_divergence.py`](../../../06_Other/simulations/star_vs_rr_divergence.py) ([why one STAR and not two](../../../06_Other/simulations/README.md#one-star-not-two)), which is held to the real engine by [`test_sim_star_model.py`](../../../STARVote_LH_tabulation_engine/tests/test_sim_star_model.py).
...
STAR is **not** Condorcet-compliant and this repo does not claim otherwise ([STAR's honest limits](../../../01_STAR/01_Learn/properties_and_limits/STAR_honest_limits.md), [Campbell–Kelly](campbell_kelly_theorem.md)). What these numbers support is narrower and more defensible than the slogan:
...
If what you want is a *guarantee* rather than a rate, that is what [Ranked Robin](../../../05_Ranked_Robin/01_Learn/ranked_robin.md) is for — and the 100.0% control column is the proof. The price of that guarantee, and why STAR does not pay it, is the subject of [what makes a good winner](../what_makes_a_good_winner.md).
