...
Read that in both directions, as the sibling page insists. It is **not** a defence that clears STAR: a real STAR election really is counted on a 0–5 ballot, so the loss is real and it lands on STAR. But it is **not a property of the automatic runoff** either, and it is inherited by every score-ballot method here. It also means the comparison is structurally generous to Ranked Robin, which reads a full-resolution ranking — the single biggest caveat on both pages, and the subject of [What the ballot can and cannot say](../../scores_and_ranks/ballot_expressiveness_measured.md), which measures how much of the gap is the ballot (at seven candidates on a spectrum: about 80% of it) and how much is the rule.

**The same page settles what this mechanism does *not* explain, which matters here.** Mechanism 3 is a score-ballot story, and the obvious next thought — *"so hand everyone a ranked ballot"* — does not survive measurement. Two reasons, both in that page's grid. Real ranked ballots are **capped** (New York City and Maine at five names, San Francisco historically at three), and a capped ranking at eleven candidates is *worse* than the 0–5 ballot by 47 points under the identical Copeland rule. And **RCV-IRV cannot spend the resolution anyway**: its efficiency barely moves between a full ranking and a six-rung ballot, because — per mechanism 1 — it only ever reads each ballot's top surviving choice, which is precisely the part of the ballot that resolution does not live in.
