...
**Totals:** 560 elections, 764 races. Full drill-down: [`races.csv`](races.csv) · [`elections.csv`](elections.csv). Related: [BV registry](BV_registry.md) · [multi-race index](multirace_elections.md) · [by method](README.md).

## Elections

| Election | Title | Races | Kind | Voters | Methods | Backing |
|---|---|--:|---|--:|---|---|
| [`2cdvm6`](https://bettervoting.com/2cdvm6/results) | BV2290 — Bloc STAR vs STAR-PR - Harborview city coun | 2 | contested (multi-race) | 108 | STAR, STAR_PR | BV |
...
| [`p2wggg`](https://bettervoting.com/p2wggg/results) | BV2286 — Three dogs and a cat — does the ballot show | 7 | contested (multi-race) | 60 | Approval, IRV, Plurality, RankedRobin, STAR | BV |
...
| [`rq2c3g`](https://bettervoting.com/rq2c3g/results) | BV2292 — Bloc STAR vs STAR-PR - fourteen candidates, | 2 | contested (multi-race) | 175 | STAR, STAR_PR | BV |
...
| [`xbk9bq`](https://bettervoting.com/xbk9bq/results) | BV2287 — Bloc STAR vs STAR-PR - the Condorcet winner | 2 | contested (multi-race) | 5 | STAR, STAR_PR | BV |
...
| [`xj8pxc`](https://bettervoting.com/xj8pxc/results) | 2026 Election Poll For Florida Voters | 4 | contested (multi-race) | 25 | STAR | BV |
| [`xpr4wk`](https://bettervoting.com/xpr4wk/results) | BV2289 — Bloc STAR vs STAR-PR - a divided majority w | 2 | contested (multi-race) | 12 | STAR, STAR_PR | BV |
...
| `1_ice_cream_counter` | OPTION 1 — The ice cream counter: seven chocolates a | 1 | single-race | 9 | STAR | LH-only |
| `2_coffee_menu` | OPTION 2 — The coffee menu: six ways to have espress | 1 | single-race | 8 | STAR | LH-only |
| `321_tennessee_blank_encoding_c4_b100` | Tennessee capital by 3-2-1 Voting (blank = Bad) | 1 | single-race | 100 | 3-2-1 | LH-only |
| `3_fizzy_or_sweet` | OPTION 3 — Fizzy or sweet? Two groupings the same si | 1 | single-race | 7 | STAR | LH-only |
| `3b_fizzy_label` | OPTION 3b — the same seven voters, asking about the  | 1 | single-race | 7 | STAR | LH-only |
| `4_pizza_toppings` | OPTION 4 — Pizza toppings: six meats and a mushroom | 1 | single-race | 8 | STAR | LH-only |
| `5_fruit_basket` | OPTION 5 — The fruit basket: seven apples and a bana | 1 | single-race | 9 | STAR | LH-only |
| `6_factions` | OPTION 6 — Left1, Center1, Right1: the labels do the | 1 | single-race | 9 | STAR | LH-only |
...
| `alaska_2022_irv_with_palin` | Alaska 2022 special (reduced model), ranked ballots  | 1 | single-race | 200 | IRV | LH-only |
| `alaska_2022_irv_without_palin` | Alaska 2022 special (reduced model), Palin deleted f | 1 | single-race | 177 | IRV | LH-only |
...
| `bloc_all_but_one` | Bloc STAR — five seats, six candidates: the election | 1 | single-race | 7 | STAR | LH-only |
| `bloc_equal_support_seat` | Bloc STAR — a seat decided by 11 voters out of 31 | 1 | single-race | 31 | STAR | LH-only |
| `bloc_finalist_wins_nothing` | Bloc STAR — making the first runoff buys you nothing | 1 | single-race | 7 | STAR | LH-only |
...
| `bloc_one_voter_council` | Bloc STAR — one voter fills a two-seat council | 1 | single-race | 1 | STAR | LH-only |
...
| `burlington_2009_irv_without_wright` | Burlington 2009 mayor, Wright deleted from every bal | 1 | single-race | 8134 | IRV | LH-only |
...
| `chicken_approval_both_defect` | Chicken / Burr dilemma — Approval, both sides defect | 1 | single-race | 100 | Approval | LH-only |
...
| `one_point_bloc_star` | One point — a single mark fills one seat and the lot | 1 | single-race | 3 | STAR | LH-only |
...
| `resign_av_holds_after_kai_c6_b5` | Resignation monotonicity — Kai resigns, and Approval | 1 | single-race | 5 | Approval | LH-only |
| `resign_av_holds_c7_b5` | Resignation monotonicity — plain Approval, before th | 1 | single-race | 5 | Approval | LH-only |
| `resign_rrv_after_hana_c4_b5` | Resignation monotonicity — Hana resigns, and Gus los | 1 | single-race | 5 | STAR_PR | LH-only |
| `resign_rrv_seated_c5_b5` | Resignation monotonicity — the three seats RRV fills | 1 | single-race | 5 | STAR_PR | LH-only |
| `resign_star_pr_after_bruno_c3_b5` | Resignation monotonicity — Bruno resigns, and Ana lo | 1 | single-race | 5 | STAR_PR | LH-only |
| `resign_star_pr_seated_c4_b5` | Resignation monotonicity — the board STAR-PR actuall | 1 | single-race | 5 | STAR_PR | LH-only |
...
| `rr_degrees_what_counts_as_a_win` | What counts as a win — the rung before the 1st Degre | 1 | single-race | 4 | RankedRobin | LH-only |
...
| `solid_coalition_quota_gets_nothing` | A full quota rates Dinah 5 and prefers her to everyo | 1 | single-race | 9 | STAR_PR | LH-only |
...
| `tie_break_ladder_matchups_eliminate_loser` | Matchups won, not preference votes — the rung that e | 1 | single-race | 5 | STAR | LH-only |
...
| `zero_support_approval` | Zero support — nobody scored anybody (Approval) | 1 | single-race | 3 | Approval | LH-only |
| `zero_support_bloc_star` | Zero support — nobody scored anybody (Bloc STAR) | 1 | single-race | 3 | STAR | LH-only |
| `zero_support_plurality` | Zero support — nobody scored anybody (Plurality) | 1 | single-race | 3 | Plurality | LH-only |
| `zero_support_ranked_robin` | Zero support — nobody scored anybody (RankedRobin) | 1 | single-race | 3 | RankedRobin | LH-only |
| `zero_support_star` | Zero support — nobody scored anybody (STAR) | 1 | single-race | 3 | STAR | LH-only |
| `zero_support_star_pr` | Zero support — nobody scored anybody (allocated) | 1 | single-race | 3 | STAR_PR | LH-only |
...
| [`pres24`](https://bettervoting.com/pres24/results) | US 2024 Presidential poll (BetterVoting pres24) - ST | 1 | single-race | 2772 | STAR | BV |
...
|---|--:|---|
| single-race | 463 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2gvwr9`](https://bettervoting.com/2gvwr9/results), [`2hqmrd`](https://bettervoting.com/2hqmrd/results) |
| contested (multi-race) | 301 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`2jrfpg`](https://bettervoting.com/2jrfpg/results), [`2p33qq`](https://bettervoting.com/2p33qq/results), [`37yf8x`](https://bettervoting.com/37yf8x/results) |
...
|---|--:|---|
| single-winner | 631 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2gvwr9`](https://bettervoting.com/2gvwr9/results), [`2hqmrd`](https://bettervoting.com/2hqmrd/results) |
| multi-winner | 133 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`3494cb`](https://bettervoting.com/3494cb/results), [`39py93`](https://bettervoting.com/39py93/results), [`3x4vrv`](https://bettervoting.com/3x4vrv/results) |
...
|---|--:|---|
| score | 406 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`2hqmrd`](https://bettervoting.com/2hqmrd/results) |
| ranked | 248 | [`2gvwr9`](https://bettervoting.com/2gvwr9/results), [`2jrfpg`](https://bettervoting.com/2jrfpg/results), [`2p33qq`](https://bettervoting.com/2p33qq/results), [`37yf8x`](https://bettervoting.com/37yf8x/results) |
...
| approval | 51 | [`4hfwqd`](https://bettervoting.com/4hfwqd/results), [`6mcgkq`](https://bettervoting.com/6mcgkq/results), `BV_Library_approval_single_winner`, `Black_Curtain_01a_c3_b5_approval` |
...
|---|--:|---|
| majoritarian | 558 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`2hqmrd`](https://bettervoting.com/2hqmrd/results) |
...
| proportional | 70 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`39py93`](https://bettervoting.com/39py93/results), [`89wwvr`](https://bettervoting.com/89wwvr/results), [`8xwx43`](https://bettervoting.com/8xwx43/results) |
...
|---|--:|---|
| bloc / at-large | 72 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`3494cb`](https://bettervoting.com/3494cb/results), [`3x4vrv`](https://bettervoting.com/3x4vrv/results), [`3yr2qd`](https://bettervoting.com/3yr2qd/results) |
| proportional | 61 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`39py93`](https://bettervoting.com/39py93/results), [`89wwvr`](https://bettervoting.com/89wwvr/results), `02a_c5_b63_proportional-allocated-score` |
...
|---|--:|---|
| STAR | 346 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`2hqmrd`](https://bettervoting.com/2hqmrd/results) |
...
| IRV | 98 | [`2jrfpg`](https://bettervoting.com/2jrfpg/results), [`37yf8x`](https://bettervoting.com/37yf8x/results), [`3xgkck`](https://bettervoting.com/3xgkck/results), [`4htk44`](https://bettervoting.com/4htk44/results) |
| STAR_PR | 56 | [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`89wwvr`](https://bettervoting.com/89wwvr/results), `02a_c5_b63_proportional-allocated-score`, `02b_c5_b63_proportional-sss` |
...
|---|--:|---|
| LH-only | 357 | `00_c3_b3_bloc-baseline-2-seats`, `00_plurality_vs_majority`, `01_c4_b2_bloc-star-2-seats`, `01_condorcet_winner` |
| BV | 308 | [`24b623`](https://bettervoting.com/24b623/results), [`26khr3`](https://bettervoting.com/26khr3/results), [`2cdvm6`](https://bettervoting.com/2cdvm6/results), [`2gvwr9`](https://bettervoting.com/2gvwr9/results) |
...
|---|---|---|
| Alaska 2022 special (reduced model), Palin d | IRV | A counterfactual profile — these ballots were never cast in this form. Publishing it as a live BetterVoting election would present a hypothetical as a real contest. |
| Alaska 2022 special (reduced model), ranked  | IRV | The ranked half of a BV-backed pair — the same electorate is already live on BetterVoting as BV2213 (k3fmwv) in its score form. This file exists to make the with/without-Palin counterfactual runnable, and its twin is a counterfactual profile that was never cast, so neither belongs on BV. |
| Burlington 2009 mayor, Wright deleted from e | IRV | A counterfactual profile — 8,134 weighted ballots that were never cast in this form. Publishing it as a live BetterVoting election would present a hypothetical as a real contest. |
