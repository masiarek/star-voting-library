...
| [**RCV-IRV engine** (pyrankvote)](../../06_Other/RCV_IRV/RCV_IRV_tabulation_engine/README.md) | Vendored RCV-IRV engine. Ranked ballots (`A>B>C`) route here automatically. Concept pages: [RCV-IRV (Hare)](../../06_Other/RCV_IRV/concepts/RCV-IRV-Hare.md). |
...
| [**ABCvoting engine**](../../06_Other/abcvoting_tabulation_engine/README.md) | Multi-winner **Approval / ABC** rules (Martin Lackner's `abcvoting`) — SPAV / PAV / Phragmén and an AV cross-check of the LH bloc-Approval count. Optional dep. |
...
| [**Simulations**](../../06_Other/simulations/README.md) | Monte-Carlo scripts: Favorite Betrayal frequency and runoff-reversal rates. |
...
| [**Demo dropbox** (watch-folder)](https://github.com/masiarek/star-voting-library/tree/master/06_Other/_demo_dropbox) | Watch-folder demo: drop a BetterVoting JSON export in, get canonical YAML + tabulation out. (GitHub link — the folder is staging space, excluded from the website.) |
| [**YAML test-case index**](../YAML_test_case_index/README.md) | **Auto-generated catalog of every election YAML, grouped by voting method** ([`STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_index.py`](../../STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_index.py); a pytest keeps it current). Browse cases by method without moving files. |
...
| [**Regenerating the derived files**](regenerating_derived_files.md) | Which of the nine generators to run after a change, in what order, and the four traps that fail silently — starting with using `.venv/bin/python` rather than bare `python3`. |
...
**Setup (once)** — clone and let [uv](https://docs.astral.sh/uv/) build the environment from the lockfile; optionally wire up the pre-commit test hook:
...
- **STAR** — single-winner Score Then Automatic Runoff (the default). → [learn](../../01_STAR/01_Learn/STAR_start_here.md)
- **Bloc STAR** / **proportional STAR** (`bloc`, `sss`, `rrv`, `allocated`) — multi-winner variants. → [Bloc](../../02_STAR_Bloc/README.md) · [proportional](../../03_STAR_PR/01_Learn/STAR_PR/README.md)
- **Approval** — score each candidate 0/1; most approvals wins. → [learn](../../04_Approval/01_Learn/approval_voting.md)
- **Ranked Robin** (`RankedRobin`, aka RCV-RR / Copeland / Consensus) — every pair of candidates compared head-to-head; best win–loss record wins. Prints the full pairwise table and flags Condorcet cycles. → [learn](../../05_Ranked_Robin/01_Learn/ranked_robin.md)
