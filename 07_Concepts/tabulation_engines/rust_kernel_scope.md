...
2. **Backfill the 48.** Every ballot-carrying case gets an answer key.
...
4. **Write down the tiebreak ladders** — per method, per engine, as prose. This is the single largest source of legitimate disagreement between correct implementations, and it is currently distributed across code, commit messages, and [one very good page](../../05_Ranked_Robin/01_Learn/rr_tiebreak_lh_vs_bv.md).
...
**Ties and lots.** The LH engine breaks a Copeland tie by total margin, then by a published `lot_numbers:` draw; BetterVoting breaks it head-to-head, then by a seeded shuffle. Two implementations can agree on every tally and still disagree on winners across a meaningful share of cases. A port must replicate the *ladder*, rung by rung, or its disagreements will be unreadable.
