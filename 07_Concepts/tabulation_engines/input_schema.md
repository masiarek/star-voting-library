...
- ~~The answer key cannot say "nobody".~~ **Closed 2026-08-21.** `expected_outcome:` is now a documented top-level key in the case-file schema, with three values — `elected` (the default), `no_winner`, `rejected` — and the two cases that needed it carry it: the [quorum failure](../../01_STAR/02_Examples/cases/quorum_fail_demo_c3_b6.yaml) that completes and seats nobody, and the [three-seats/three-candidates race](../../02_STAR_Bloc/02_Examples/cases/bv2269_t488h9_race_nobody_can_lose.yaml) the engine refuses outright. It flows straight into `expected.outcome` here. Both halves are enforced, and by the same file — [`tests/test_expected_outcome.py`](../../STARVote_LH_tabulation_engine/tests/test_expected_outcome.py) — because the pre-commit hook runs `check_repo_hygiene.py` as `… || true`, so a hygiene check nothing calls from a test prints a green line and gates nothing. That test calls the lint (refusing a self-contradicting key, `no_winner` alongside a winners list) *and* proves the lint is not vacuous by feeding it each malformed shape; it also holds the *engine* to the claim — a `no_winner` file must exit 0, announce no winner banner, and name nobody; a `rejected` file must exit 1 with a plain-language error, no traceback, and no `_tabulated` mirror left behind. Every answer key in the library now asserts something.
