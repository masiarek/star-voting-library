...
- **Run BetterVoting locally** — the two setups (dev server vs Docker) and the gotchas that cost time (AirPlay port, Keycloak, `crypto.randomUUID`, shared-package rebuilds) → [running BetterVoting locally](running_bettervoting_locally.md).
- **Contribute a change** — the fork → PR workflow and lessons from the first contribution ([PR #1419](https://github.com/Equal-Vote/bettervoting/pull/1419) — split by a maintainer, its CSV fixes merged as [#1428](https://github.com/Equal-Vote/bettervoting/pull/1428) with authorship intact, its JSON-v2 half deferred), since you don't have direct push to `Equal-Vote/bettervoting` → [contributing a change](contributing_to_bettervoting.md).
...
winner. [`tools_adam/bv_replay_tiebreak.py`](../../../../STARVote_LH_tabulation_engine/tools_adam/bv_replay_tiebreak.py) is a stdlib-only Python port of TinyRand + this
