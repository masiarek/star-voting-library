...
| [#1421](https://github.com/Equal-Vote/bettervoting/issues/1421) | **None of the Above** (a real `c-nota` candidate) wins → seated with no special handling. **Closed 2026-08-19 as by design** — next steps belong to the convening body's bylaws, explicitly out of scope for BV | BV215 · [26khr3](https://bettervoting.com/26khr3) · **closed (by design)** |
| [#1478](https://github.com/Equal-Vote/bettervoting/issues/1478) | A **partial ballot whose non-blank marks are all equal** (a lone `1` + two blanks) is excluded from the tally as an abstention — `nTallyVotes 2 / nAbstentions 2` on 4 ballots. **See the correction below: the dropped ballot is the all-5s one, not the partial one** | BV2105 · [r4dqvd](https://bettervoting.com/r4dqvd) · re-check BV2105-r2 · [w3vvff](https://bettervoting.com/w3vvff) |
| [#1508](https://github.com/Equal-Vote/bettervoting/issues/1508) | A **fully marked ballot whose marks are all equal** (`5,5` — maximum support for everyone) is excluded from the tally, so its scores never reach the totals: `nTallyVotes 3 / nAbstentions 2` on 5 ballots, A published as **9** where the ballots say **14**. The minimal form of the defect — 2 candidates, 5 ballots — with BV's own source quoted. Filed by us 2026-08-09 | BV2283 · [hb4qvv](https://bettervoting.com/hb4qvv/results) · June capture [3w6v4b](https://bettervoting.com/3w6v4b) · [lesson](../../../01_STAR/04_Real_Elections/pet_real_bv_election/small_abstention_c2_b5_lesson.md) |

> **Correction to the #1478 row (2026-08-09).** The rule is real, but that election's *dropped* ballot was misidentified, and #1478 says so on the ticket. Reading `w3vvff` back live: it reports Vanilla **3**, Chocolate **5**, Strawberry **4**, and `c.score` is a **sum** (`tallyVotes.reduce((score, vote) => score + vote.marks[c.id], 0)`), so those totals are exactly `[1, null, null]` + `[2, 5, 4]`. The **partial ballot was counted**; the ballot dropped alongside the blank was the **all-5s** one. That also follows from the code — a ballot record carries a slot per candidate, so the partial ballot's marks are `[1, 0, 0]`, which is not all-equal. Same rule, same `nTallyVotes 2 / nAbstentions 2`, different ballot. The probe that *does* discriminate is the fully-marked flat ballot of #1508.
