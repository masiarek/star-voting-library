...
- **[The election contract — the input half](input_schema.md)** — the other side of the same door: a normalized JSON description of the *contest*, with one illustration per method, so two engines provably read the same election from the same bytes. Written because the result contract quietly assumes that and cannot check it — PyYAML (YAML 1.1) and every Rust/Go/JS parser (YAML 1.2) resolve `No`, `12:30` and `007` differently from identical bytes. `--emit-election-json` puts all 593 case files through it, and on the 586 with countable ballots the ballots rebuilt from the JSON are identical to the ones the engine parsed. Also grades the prior art (RCTab, ABIF, NIST CDF, PrefLib) rather than deferring to it, and says which of the nine kernel goals the design serves.
