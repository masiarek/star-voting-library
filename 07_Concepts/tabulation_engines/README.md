...
- **[The election contract — the input half](input_schema.md)** — **proposed, nothing emits it yet.** The other side of the same door: a normalized JSON description of the *contest*, with one illustration per method, so two engines provably read the same election from the same bytes. Written because the result contract quietly assumes that and cannot check it — PyYAML (YAML 1.1) and every Rust/Go/JS parser (YAML 1.2) resolve `No`, `12:30` and `007` differently from identical bytes. Also grades the prior art (RCTab, ABIF, NIST CDF, PrefLib) rather than deferring to it, and says which of the nine kernel goals the design serves.
