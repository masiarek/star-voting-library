...
The payoff is that the refined rules turn out to be *unanimous*: Minimax, Ranked Pairs, Schulze, Split Cycle, Stable Voting and RCV-IRV all elect **Ranked Pairs**, with Schulze and Minimax each voting for a rival over itself. The one rule that cannot decide is **Copeland's Rule**, which reads only wins and losses — all three cycle members go 3–1, so it returns a three-way tie. This repo's [Ranked Robin](../../05_Ranked_Robin/01_Learn/ranked_robin.md) *is* Copeland, so it hits that tie and breaks it on the [1st Degree](../../05_Ranked_Robin/03_Criteria/rr_tiebreaks/degrees_of_ties.md) — the sum of win margins over the other tied finalists, which here reads Schulze +103, Ranked Pairs +45, Minimax −148 — landing on **Schulze Method**, a different winner from every margin-reading rule. That divergence is the clearest demonstration in the library of why the refined rules were invented.
