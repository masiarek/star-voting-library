...
Both engines' five-star rung counts ballots at the **scale maximum only** (`score == 5`; BV's code is literally `=== 5`), which is why it can be a [dead rung](../../01_STAR/03_Criteria/tie_break_dead_rung/README.md) — no fives to count, the tie drops straight to the floor without ever consulting the 4s. The 3+-way skip on BV's rung 1 is deliberate, [confirmed on #1379](https://github.com/Equal-Vote/bettervoting/issues/1379) — but it is a departure from the **published** protocol, not a reading of it. Equal Vote's [Official Tiebreaker Protocol](https://www.starvoting.org/ties) states rung 1 for a tied group of *any* size: with only two tied it is the majority-preferred candidate, and with more than two the tied candidates are compared head to head, “eliminating the candidate(s) who lost the most match-ups”, **repeated as needed** until two can advance. **Neither engine implemented that rung until 2026-08-21**, in two different ways: BetterVoting skips it, and LH ran a *different statistic* under its name — vendored `starvote` summed each ballot's pairwise preferences (compare-then-aggregate) where the protocol counts matchups won (aggregate-then-compare). The two agree at two tied candidates and diverge at three or more, where the old tally could score a candidate who loses **every** head-to-head equal to candidates who win theirs, letting the five-star rung advance him. LH now implements the published rung ([`_matchups_won_round`](../../STARVote_LH_tabulation_engine/starvote/__init__.py), guarded to 3+, mirrored in `resolve_finalists()` so the matrix markers cannot disagree with the count); the probe that pins it is [Matchups won, not preference votes](../../01_STAR/03_Criteria/tie_break_ladder/cases/cases_pages/tie_break_ladder_matchups_eliminate_loser.md), and the story — including the 2023 upstream report that fixed the *other* counter in the same function — is [Matchups won vs. preference votes](../../01_STAR/01_Learn/Tie_Breaking_STAR/matchups_won_vs_preference_votes.md). BV's own [help page](https://docs.bettervoting.com/help/ties.html) restates the protocol its code implements (3+ → straight to five-star), so that divergence is documented on both sides without being named as one. Note also that `starvoting.org/ties` now 301-redirects to a client-rendered `bettervoting.com/ties`; the wording above is from the [2025-09-12 capture](http://web.archive.org/web/20250912011025/https://www.starvoting.org/ties) (checked 2026-08-21). Equal Vote also publishes an optional **[Condorcet Tiebreaker](../../01_STAR/01_Learn/Tie_Breaking_STAR/condorcet_tiebreaker.md)** for hand counts (matches-won → preference votes → win margin → random) — a third ladder, implemented by neither engine here.
...
The [result contract](result_schema.md) draws the operational conclusion from all of the above: `tiebreaks[].rung` is deliberately free text, because rung *names* legitimately differ between correct engines. A conformance run should compare (1) the tied set, (2) the outcome, and (3) whether a rung fired **at all** — an empty `tiebreaks: []` is a positive claim that the ballots alone decided, and an engine that reaches for a tiebreak where the reference did not has a bug even when it lands on the same winner. And date your fixtures three times over: any Ranked Robin result captured before 2026-08-19 predates the 1st-Degree rung; any *score* result captured before 2026-08-21 predates the contract reporting the **lot** rung at all; and a result from earlier the same day predates it reporting the **Automatic Runoff's deterministic rungs**, which left 16 more cases claiming `tiebreaks: []` on an election their own report narrated a tiebreaker for ([the sweep that found it](../topics/ties/coarse_ballots_and_the_tie_ladder.md)) — on 23 cases here, `b484mbm_tie_every_rung` and `bv750_tie_breaking_bloc` among them, `tiebreaks: []` was being emitted for a seat the lot had bought. Both are stale rather than divergent; re-emit before reading a difference as a disagreement.
...
- [Coarse ballots and the tie ladder](../topics/ties/coarse_ballots_and_the_tie_ladder.md) — 445,154 small elections swept for tie categories this page does not list; found none, and found one reporting gap in the contract
