...
| **Return the tied set** and decline | `pref_voting` the library, `abcvoting`'s irresolute rules, Range and an exhausted Majority Judgment in the grade tool | n/a — no choice is made | yes, by construction |
...
- **LH** (`allocated_score_voting` / `sequentially_spent_score` / `reweighted_range_voting` in vendored `starvote`): a tie on **weighted** total score in a selection round goes **straight to the lot**. One known wording defect: the shared lot banner still prints STAR's text ("the deterministic rungs (pairwise / score, then five-star) all came back equal") on a PR-family tie where no such rungs exist — see [the Lackner–Skowron shadow case](../../03_STAR_PR/02_Examples/cases/cases_pages/lackner_skowron_shadow_star_pr_c7_b12.md)'s mirror for it firing. The winner is right; the sentence is borrowed.
- **BetterVoting** (`AllocatedScore.ts`): highest weighted score (exact `Fraction` equality) → shuffle order. **Plus a reporting defect confirmed by direct execution (2026-08-20, not yet filed upstream):** the round winner is unconditionally pushed into `tied[]`, and the final `tieBreakType` check compares elected against `tied` by object identity — so a STAR-PR race with **no tie anywhere** still reports `tieBreakType: "random"` with the winners listed as the tied set. Treat BV's STAR-PR tie *reporting* as unreliable; the tally itself is fine.
...
- **`abcvoting`** ([`abc_tabulation.py`](../../06_Other/abcvoting_tabulation_engine/abc_tabulation.py)): a split the wrapper's own docstring gets only two-fifths right. `av` / `sav` / `pav` default to **irresolute** — every tied committee is returned and the report prints `[N tied committees]`. But `seqpav` and `seqphragmen` default to **resolute** in the library, and their tie-break is *smallest candidate index* — which, the way this repo loads elections, means **left-to-right ballot-header column order** — applied silently, with the tie recorded only in a `detailed_info` structure the wrapper never reads. A `seqpav` committee from this repo's report may therefore have been tie-broken by column order with no marker anywhere on the page (flagged 2026-08-20; the wrapper predates the finding).
...
- **Column order is the hidden floor in four places, disclosed in one.** The LH engine's no-`lot_numbers` fallback is ballot-column order *and the report says so*. The other three are silent: `pyrankvote`'s dead-ladder residual (row/column order), the RCV wrapper's equal-score-to-ranking rule, and `abcvoting`'s resolute `seqpav` / `seqphragmen` (smallest index = header column).
...
- **BetterVoting reports one `tieBreakType` per race, escalated to the worst rung reached** — and three method-specific gaps found on 2026-08-20, none yet filed upstream: STAR never populates `tied[]` at all (the information lives in `tieBreakType` and the logs); a bloc race keeps only the **final** seat's tiebreak; STAR-PR reports `random` even with no tie (the confirmed defect above); and an IRV elimination tie among three or more standing candidates is broken without being flagged. Its floor, on the other hand, is better than its name: "random" is a seeded, recorded, replayable shuffle — see [the recorded-tiebreak page](../../05_Ranked_Robin/01_Learn/rr_tiebreak_lh_vs_bv.md) for why that is still weaker than a published lot.
- **The LH PR-family lot banner borrows STAR's wording** (names pairwise/five-star rungs that never ran on that path) — a known cosmetic defect, tracked for a fix; trust the winner, not that sentence.
