...
**Fourth run: the new transfer block.** When the RCV-IRV report gained a [transfers-and-inactive-ballots block](../../06_Other/RCV_IRV/RCV_IRV_tabulation_engine/README.md#transfers-and-inactive-ballots) — where each eliminated candidate's ballots went, how many stopped counting, and the winner's total measured against *all* ballots cast — RCTab is what made those numbers evidence rather than our own arithmetic agreeing with itself. Three cases, including Minneapolis 2017 (104,484 real ballots, 18 candidates): every transfer destination matches, every exhausted count matches, and so does every majority threshold as the denominator shrinks — `26 → 26 → 22 → 15` on the [forced-exhaustion ceiling](../../method_comparisons/paradoxes_and_whoops/bv2183_dfw8rj_forced_exhaustion_ceiling.md), `46790 → 40838` in Minneapolis's last two rounds. What does *not* line up is the round **numbering**: `pyrankvote` batch-eliminates and RCTab as configured does not, so Minneapolis is 9 rounds here and 18 there. The transfers coincide wherever the elimination sequences do — a difference of grouping, already documented above, not of arithmetic.
