...
**1.1.0 (2026-08-21)** was the first bump. `tiebreaks[]` gained the optional `round` field — the additive half — and, the half that matters, the score path began reporting the **lot** rung at all. Before it, the only tie the builder could see was the finalists ladder that `resolve_finalists()` replays for single-winner STAR, so a Bloc or PR seat bought by lot, and a single-winner Automatic Runoff bought by lot, both emitted `tiebreaks: []` — which point 4 above defines as the positive claim that the ballots alone decided. **23 cases in this library made that claim falsely**, among them `lot_tiebreak_published_order.yaml` and the whole [dead-rung set](../../01_STAR/03_Criteria/tie_break_dead_rung/README.md), whose entire subject is the lot. Any result captured before that date is stale rather than divergent, exactly like the Ranked Robin fixtures above: re-emit it before reading a difference as a disagreement.

**1.2.0 (2026-08-21)** closed the other half of the same gap, the same week and for the same reason. 1.1.0 taught the builder to see the **lot**; it still could not see a runoff tie that a *deterministic* rung resolved, because those fire no lot event and `resolve_finalists()` replays the other ladder. So an election whose printed report says in full *"Automatic Runoff Round: First tiebreaker — the highest-scoring candidate wins"* emitted `tiebreaks: []`. **16 committed cases did that** — among them [`bv830_vb3xv2_no_condorcet_tie_score`](../../01_STAR/03_Criteria/tie_break_ladder/bv830_vb3xv2_no_condorcet_tie_score.md) and `tie_break_04_runoff_five_star_breaks`, files *named for the rung the JSON did not mention*. The fix is the mirror of the one that already existed: `resolve_runoff()` replays STAR's second ladder — preference round, then total score, then the scale-maximum count, then the lot — through starvote's own round functions, so the replay cannot disagree with the count. Twelve of the sixteen were `rung: "score"`, four `"five-star"`. Found by [the coarse-ballot tie sweep](../topics/ties/coarse_ballots_and_the_tie_ladder.md), which is the kind of thing a contract this claim-heavy needs pointed at it.
...
- **Below the lot, a MULTI-winner count's ladder is invisible.** `tiebreaks` names every tie the *lot* settled, on every seat; for single-winner STAR it also names the rung that settled the finalists and the rung that settled the runoff, because `resolve_finalists()` and `resolve_runoff()` replay both ladders against starvote's own round functions. There is no equivalent replay for a Bloc STAR round or a PR round: those rungs run inside starvote's counting functions, which report nothing back, and rebuilding them in the builder would mean re-deriving the count, which this contract does not do anywhere. So a Bloc seat settled at the five-star rung still reads here as a seat nothing was broken for. It is the one remaining way the array can understate what happened — and [the coarse-ballot sweep](../topics/ties/coarse_ballots_and_the_tie_ladder.md) measures how often, by driving Bloc rounds into ties on purpose and diffing the engine's narration against the contract.
...
- **The input format has no published schema — and this contract assumes one.** Everything above compares two *counts*; it takes on faith that the two engines read the same *election*, and there is no field here that can check it. `source.sha256` proves the bytes matched, which is not the same claim: PyYAML resolves YAML 1.1, so an unquoted `No` on a ballot measure arrives as `False` and `12:30` as `750`, while every Rust, Go and modern JS parser resolves YAML 1.2 and reads both as strings — identical bytes, two different elections, and the *more correct* implementation is the one scored as divergent. A design for the missing half, with one illustration per method, is now published at [the election contract](input_schema.md); it is proposed rather than built, and the one thing it waits on is an `--emit-election-json` mode on this engine. Pydantic models over the case file would close the same gap from the other side.
