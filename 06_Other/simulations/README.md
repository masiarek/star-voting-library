...
**Thinking about a *harness* rather than another script?** [Goals, requirements, and the existing landscape](SIMULATOR_REQUIREMENTS.md) surveys the fifteen simulators other people have already written — including the one in Julia that covers all six of this library's methods — derives the requirements from what they assume, and measures whether Rust would help (2.9× for sampling, 238× for exhaustive search).
