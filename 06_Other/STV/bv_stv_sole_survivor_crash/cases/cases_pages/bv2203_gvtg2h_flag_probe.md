...
1:Bronte>Camus, 3:Camus>Dickens; 2 seats; hand-count Droop quota 4),
created with
...
NOTE — which Droop quota: the elimination sequence above is the one
BetterVoting's tabulator walks under the hand-count quota of 4, and it
is the shape that crashes it. The LH report below applies the other
standard Droop quota, the exact 9/3 = 3.00, and on that rulebook the
second seat fills with NO elimination round at all — so don't expect
the sole-survivor endgame to appear in the table. Same seats either
way; the crash is BV's, and it is real.
