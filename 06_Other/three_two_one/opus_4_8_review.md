...
> **How to read this page.** The rest of the library states things it can verify — a tabulation, a criterion, a simulated rate with its model attached. This page does something different: it *judges*. Judgements can't be regression-tested, so they're quarantined here, signed and dated, rather than mixed into the concept pages. If you want the neutral treatment, read [3-2-1 voting](../../07_Concepts/topics/three_two_one_voting.md) and [the method folder](README.md) instead.

## Provenance and lean (read first)

- **What I read:** this repo's [3-2-1 concept page](../../07_Concepts/topics/three_two_one_voting.md), [the folder README](README.md) with Quinn's spec, and the [clean-room engine](three_two_one_tabulation.py) verified against Quinn's own reference vectors.
...
Three levels means **the deciding step often has nothing to read**. If two allies are both honestly *Good*, no voter distinguishes them, and step 3 is a coin flip — the repo's own [worked example](../../07_Concepts/topics/three_two_one_voting.md) shows exactly this.
...
3-2-1 is an **orphan**. Quinn designed it and [died in March 2025](../../07_Concepts/topics/in_memoriam_jameson_quinn.md); the Center for Election Science campaigns for Approval, the Equal Vote Coalition for STAR and Ranked Robin. Nobody is carrying it.
