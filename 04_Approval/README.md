...
**Where the committee rules stand on the criteria** — [03_Criteria](03_Criteria/README.md) recomputes Lackner & Skowron's Table 3.1 for all thirteen approval-based committee rules: Pareto efficiency, committee monotonicity, support monotonicity, consistency, strategyproofness and complexity, with every failure demonstrated by a runnable witness rather than transcribed. Two rows are worth the trip on their own — **AV passes every column, and SAV passes every column but strategyproofness**; every rule with a real proportionality claim buys it by failing something.
