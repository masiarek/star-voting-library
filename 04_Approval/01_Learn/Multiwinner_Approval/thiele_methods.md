...
- **But RRV is score-PAV.** **Reweighted Range Voting** reweights each ballot by a harmonic-style divisor `1 / (1 + (score already awarded to winners)/max)` before each seat. On **0/1** ballots that divisor *is* the seq-PAV weight — which is exactly why, in the [shadow STAR](../../02_Examples/multiwinner/lackner_skowron_shadow_star.md), **RRV recovered PAV's `{A,B,C,F}`** while Allocated/SSS stayed at `{A,B,C,D}`. RRV is the **score-ballot generalisation of sequential PAV**; the harmonic "diminishing returns" idea is the same, lifted from *counts* to *scores*.
