...
The provenance is unusually recent for a voting method. Brams and Fishburn's [*Approval Voting*](../../00_start_here/books/rated_and_score_methods.md) (2007) credits the idea to **five different groups who arrived at it independently during the 1970s** — no single inventor, which is itself a mild argument that the rule is a natural thing to land on. The multi-author reference is Laslier and Sanver's edited collection (2010), which is also where the *critiques* live; both books are on the [rated & score methods shelf](../../00_start_here/books/rated_and_score_methods.md).

## The six arguments made for it

Restated from the survey (which frames them as the case for Approval as an improvement on [Choose-One](../../00_start_here/topics/plurality.md) in political elections), with where this library can check each one:
...
| 2 | **It fixes plurality's worst failure** — one candidate at the minority end of a spectrum defeating several who split the majority end. | The [spoiler effect](../../00_start_here/topics/spoiler_effect.md) and the [vote-splitting set](../../method_comparisons/split_voting/); see the real election below. |
...
| 5 | **Likely to elect the [Condorcet winner](../../00_start_here/topics/condorcet/) when one exists** (Beaujard et al. 2014, who also argue Approval favors "consensual" candidates near the middle of a multidimensional issue space — a generalization of argument 2). | *Likely*, not always — the counterexample is [in this repo](#the-caveat-that-matters-in-a-real-election) and is worked at the bottom of this page. |
...
| 3 | **It violates "one person, one vote."** | **Weak** — the survey reads this as an argument of convenience rather than conviction. See [one person, one vote](../../00_start_here/topics/one_person_one_vote.md): Approval passes the mathematical version of the standard (the [Test of Balance](../../01_STAR/concepts/properties_and_limits/equally_weighted_vote.md)). |
...
- **Every profile has a [Condorcet winner](../../00_start_here/topics/condorcet/).** Define "x beats y" as *more voters strictly prefer x to y than the reverse*. On dichotomous ballots that is precisely "more voters approve x than approve y" — which is transitive and always has a maximum. No cycles are possible.
...
- **William S. Zwicker, "Introduction to the Theory of Voting,"** in *Handbook of Computational Social Choice* (Brandt, Conitzer, Endriss, Lang & Procaccia, eds., Cambridge University Press, 2016) — the approval-voting section, including the arguments and criticisms above, the three readings of "approve," and the *Approval = Borda = Condorcet* result. **Lean:** neutral; the standard academic reference, and the same chapter this repo leans on for [May's theorem](../../00_start_here/topics/mays_theorem.md), the [SWF/SCF distinction](../../00_start_here/topics/social_welfare_function.md), [what a method reads](../../00_start_here/topics/what_a_method_reads.md) and the [cycle–cocycle decomposition](../../00_start_here/topics/cycle_cocycle_decomposition.md).
- Works it cites, and which this page names: **Brams & Fishburn**, *Approval Voting* (2007) and **Laslier & Sanver** (eds.), *Handbook on Approval Voting* (2010) — both on the [books shelf](../../00_start_here/books/rated_and_score_methods.md); **Balinski & Laraki** (2010) on the ambiguity as a fatal flaw (see [majority judgment](../../00_start_here/voting_paradoxes/majority_judgment.md)); **Beaujard et al.** (2014) on Condorcet efficiency and consensual candidates; **Duddy et al.** (2013) on the mean-utility threshold; **Laslier** (2009) on the expected-winner threshold.
...
- [Preference vs support](../../00_start_here/scores_and_ranks/preference_vs_support.md) · [scores vs ranks](../../00_start_here/scores_and_ranks/scores_vs_ranks.md) — the two questions a ballot can ask
- [Gibbard–Satterthwaite](../../00_start_here/topics/gibbard_satterthwaite_theorem.md) — why "resistant" is the strongest any method gets
