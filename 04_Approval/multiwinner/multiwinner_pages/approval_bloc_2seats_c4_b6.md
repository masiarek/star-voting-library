...
*Generated from [`approval_bloc_2seats_c4_b6.yaml`](../approval_bloc_2seats_c4_b6.yaml) — do not edit by hand. Regenerate: `python scripts/build_yaml_pages.py`.*
...
A cohesive majority (4 of 6 voters, approving Amy and mostly Ben) takes
BOTH seats; the minority's candidates (Cora, Doug) win nothing. That's
the lesson: bloc Approval is majoritarian, not proportional — same
trade-off as Bloc STAR (02_STAR_Bloc). Proportional rules on the SAME
ballots (SPAV, PAV, seq-Phragmén — run them via
abcvoting_tabulation_engine/) instead elect Amy + Cora, giving the
minority its seat; see 00_start_here/Approval_Voting/approval_multiwinner.md.
...
```text
Amy,Ben,Cora,Doug
1,1,0,0   # voter 1 — majority faction: Amy and Ben
