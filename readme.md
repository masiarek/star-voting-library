...
5. **Publish.** `STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py` renders each election into a
...
| Area | What's there |
|---|---|
| [**STAR engine** (LH `starvote` fork)](STARVote_LH_tabulation_engine/README_STARVote_LH_tabulation_engine.md) | The STAR engine (single-winner STAR, Bloc / proportional STAR, Approval, **Ranked Robin**), reporting options, the preference matrix, the `[Divergence from STAR]` comparison, the optional `show_runoff_percent` runoff summary, and the `lot_numbers` tie-break. Vendored fork of `starvote`; see [`FORK_NOTES.md`](STARVote_LH_tabulation_engine/FORK_NOTES.md). |
| [**RCV-IRV engine** (pyrankvote)](06_Other/RCV_IRV/RCV_IRV_tabulation_engine/README_RCV_IRV_tabulation_engine.md) | Vendored RCV-IRV engine. Ranked ballots (`A>B>C`) route here automatically. Concept pages: [RCV-IRV (Hare)](00_start_here/RCV_IRV/RCV-IRV-Hare.md). |
| [**pref_voting cross-check engine**](STARVote_LH_tabulation_engine/tools_adam/pref_voting_tabulation_engine/README_pref_voting_tabulation_engine.md) | Independent cross-check engine — wraps Eric Pacuit's `pref_voting` to verify the LH engine's Condorcet / IRV / Plurality (and report Copeland = Ranked Robin). Optional dep. |
| [**abcvoting engine**](abcvoting_tabulation_engine/README_abcvoting_tabulation_engine.md) | Multi-winner **Approval / ABC** rules (Martin Lackner's `abcvoting`) — SPAV / PAV / Phragmén and an AV cross-check of the LH bloc-Approval count. Optional dep. |
| [**YAML library — imports & converter**](YAML_library/README_YAML_library.md) | BetterVoting JSON → YAML converter ([`01_convert_json_yaml.py`](YAML_library/1_positive/01_convert_json_yaml.py)) and imported elections (`YAML_library/1_positive/`). |
| [**YAML library — negative fixtures**](YAML_library/README_YAML_library.md) | Malformed fixtures (`YAML_library/2_negative/`) — every one must fail with the right plain-language error (see [Validation philosophy](#validation-philosophy) below). |
| [**Example elections** (01_STAR … 06_Other)](01_STAR/README_01_STAR.md) | Hand-authored example elections, grouped by content type: [STAR](01_STAR/README_01_STAR.md), [Bloc STAR](02_STAR_Bloc/README_02_STAR_Bloc.md), [proportional STAR](03_STAR_PR/README_03_STAR_PR.md), [Approval](04_Approval/README_04_Approval.md), [Ranked Robin](05_Ranked_Robin/README_05_Ranked_Robin.md), [method comparisons](method_comparisons/README_method_comparisons.md), [other methods](06_Other/README_06_Other.md). |
