...
| [**pref_voting cross-check engine**](pref_voting_tabulation_engine/README_pref_voting_tabulation_engine.md) | Independent cross-check engine — wraps Eric Pacuit's `pref_voting` to verify the LH engine's Condorcet / IRV / Plurality (and report Copeland = Ranked Robin). Optional dep. |
...
| [**YAML test-case index**](00_start_here/YAML_test_case_index/README_YAML_test_case_index.md) | **Auto-generated catalog of every election YAML, grouped by voting method** ([`scripts/build_yaml_index.py`](scripts/build_yaml_index.py); a pytest keeps it current). Browse cases by method without moving files. |
