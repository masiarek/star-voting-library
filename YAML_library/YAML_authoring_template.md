...
| `paradoxes` | optional, but **the paradox index sees nothing else** | List of paradox tags (valid tags: the pages under [`07_Concepts/voting_paradoxes/`](../07_Concepts/voting_paradoxes/README.md)). The sole input to the auto-generated [voting-paradoxes index](../07_Concepts/voting_paradoxes/README.md) (`build_paradox_index.py`) — a case demonstrating a paradox without this key is invisible there. |
...
> **Richer, converter-produced files.** Elections imported from BetterVoting (via the [JSON→YAML converter](1_positive/01_convert_json_yaml.py)) carry a fuller shape than a hand-written case: candidates as objects with **explicit IDs** (not just a name row), more `election_*` context, and an **`expected_results:`** block that pins per-round detail. You don't hand-write these — the converter emits them — but the flat, hand-authored fields above are all you need to write a case yourself.

## `voting_method` values

| Value | Ballot | Counts as |
|---|---|---|
| `STAR` | scores 0–5 | [Score Then Automatic Runoff](../01_STAR/01_Learn/STAR_start_here.md) (single-winner default) |
| `Approval` | `0`/`1` only | [Most approvals wins](../04_Approval/01_Learn/approval_voting.md) |
| `RankedRobin` (aka `RCV_RR`, `Copeland`, `Consensus`) | scores 0–5 (read as an order) | [Head-to-head round robin](../05_Ranked_Robin/01_Learn/ranked_robin.md); best win–loss record wins |
| `RCV_IRV` | ranked, `A>C>B` | [Instant runoff](../06_Other/RCV_IRV/concepts/RCV-IRV-Hare.md) (elimination rounds) |
| `bloc` | scores 0–5 | [Bloc STAR](../02_STAR_Bloc/README.md) (multi-winner, majoritarian) |
| `sss` | scores 0–5 | [Sequentially Spent Score](../03_STAR_PR/01_Learn/STAR_PR/README.md) (proportional) |
| `rrv` | scores 0–5 | [Reweighted Range Voting](../03_STAR_PR/01_Learn/STAR_PR/README.md) (proportional) |
| `allocated` | scores 0–5 | [Allocated Score](../03_STAR_PR/01_Learn/STAR_PR/README.md) (proportional) |
...
- **Other ballot formats.** These same rows in the election-methods [ABIF interchange format](../07_Concepts/scores_and_ranks/abif_format.md) look like `Allie/5 =Billy/5 >Candace/4` — one dense self-describing line per ballot; decoded and compared there. (ABIF ≈ this `ballots:` block; the rest of the file stays ours.)

## House style (so your case fits the library)

- **Keep it small.** The fewest ballots that make the point — a handful of individual voters beats 100 weighted ones. Scale up only if percentages or proportional seats genuinely need it. See [TIPS_choosing_voter_counts.md](../07_Concepts/tips/TIPS_choosing_voter_counts.md).
...
Live examples to crib from: [01_STAR — single-winner STAR Voting](../01_STAR/README.md) — start with [`bv2184_fyy886_lunch_vote.yaml`](../01_STAR/02_Examples/cases/bv2184_fyy886_lunch_vote.yaml) (the clean beginner example) and [`03c_c6_b8_style-gallery.yaml`](../01_STAR/02_Examples/cases/03c_c6_b8_style-gallery.yaml) (every optional field in use).
