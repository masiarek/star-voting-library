...
- **Every relocated page keeps a permanent redirect** in `mkdocs.yml`'s `redirect_maps` (120 of them). Published URLs are quoted in permanent BetterVoting election descriptions that cannot be edited after the election goes live, so those redirects are not housekeeping — deleting one creates an unfixable 404. Moving concept pages again? Use [`tools_adam/scripts/migrate_concept_links.py`](../STARVote_LH_tabulation_engine/tools_adam/scripts/migrate_concept_links.py), which resolves relative links against each source file rather than blind-replacing strings, and add the new redirects.
