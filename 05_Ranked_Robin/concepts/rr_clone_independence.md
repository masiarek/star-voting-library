...
- LH-only case pair (deterministic teaming via margin): [`clone_teaming_01_pre`](../clone_independence/cases/clone_teaming_01_pre.yaml) → A (lot), [`clone_teaming_02_post`](../clone_independence/cases/clone_teaming_02_post.yaml) → A1 (margin).
- BetterVoting companions (the divergence, where teaming fails): [BV2142 — pre](../clone_independence/bv2142_4gfwdq_clone_cycle_pre.md) (BV random → C) and [BV2143 — teaming](../clone_independence/bv2143_9pr3wr_teaming_fails.md) (BV head-to-head → C, teaming fails). Confirmed live: BV `4gfwdq` / `9pr3wr`.
