...
- **Copeland's method** — the **algorithm** underneath: most head-to-head wins, ties count ½. With Ranked Robin's sum-of-margins tiebreak the precise technical name is **Copeland//Borda** (Copeland, completed by a Borda-style margin score over the tied set) — not to be confused with the Borda count itself, which is a different method. → [Copeland's method (Wikipedia)](https://en.wikipedia.org/wiki/Copeland%27s_method)
...
Thirteen voters, four candidates on a left→right line (Ada, Ben, Cara, Dan). Run it: [`ranked_robin_consensus_center.yaml`](../_main/cases/ranked_robin_consensus_center.yaml) (or paste the `count:A>B>C` block into [LeGrand's calculator](https://www.cs.angelo.edu/~rlegrand/rbvote/calc.html)).
...
> **The LH engine now tabulates Ranked Robin first-class.** Set `voting_method: RankedRobin` (aliases `RCV_RR` / `Copeland` / `Consensus`) and the engine prints the **round-robin report** — ballots, the full pairwise table, and each candidate's win-loss record — instead of the RCV-IRV elimination rounds: ``` python STARVote_LH_tabulation_engine/starvote_larry_hastings.py 05_Ranked_Robin/_main/cases/ranked_robin_consensus_center.yaml ``` It flags a **cycle** (when the top candidates tie on wins) and points to [cycle resolution](cycle_resolution.md). For an *independent* Copeland cross-check, [`ranked_robin_report.py`](../../STARVote_LH_tabulation_engine/tools_adam/pref_voting_tabulation_engine/ranked_robin_report.py) (in the `pref_voting` engine) computes the same result a second way.
...
Add `options: { show_matrix: true }` to pull that matrix onto the screen too — which is what [`ranked_robin_consensus_center.yaml`](../_main/cases/ranked_robin_consensus_center.yaml) does, since the matrix is the point of that worked example.
...
| [Most matchups won ≠ Condorcet winner](../most_wins_vs_condorcet/) | its mirror image — strictly the most wins, and still beaten head-to-head; only Copeland decides |
| [Tiebreaks](../rr_tiebreaks/) | the Equal Support column and the full ladder down to lot order |
| [Burial](../burial/) | the signature wart, worked as a sincere/buried pair on both engines |
| [STAR vs RR — 30 divergence samples](../star_vs_rr_divergence/) | 30 elections where STAR and RR elect different winners, each stating its own cause |
...
- [Ranked Robin (electowiki)](https://electowiki.org/wiki/Ranked_Robin) *(the canonical definition of the "Ranked Robin" name — community wiki, Equal-Vote-adjacent; includes the method's history and the full four-degree Equal Vote tie-break hierarchy)*
