...
[Ranked Robin](../../01_Learn/ranked_robin.md) is usually explained as "whoever wins the most head-to-head matchups wins." That is a shorthand, and it is very nearly always right. This page is about the case where it isn't — where the actual rule, the **Copeland score**, elects someone the shorthand would not.
...
And note what Alice's record actually contains: **a loss.** Carmen beat Alice 21–9, a thumping. Ranked Robin still elects Alice, because nobody in this election beat everybody — there is no [Condorcet winner](../../01_Learn/ranked_robin_vs_condorcet.md) at all. The [Smith set](../../../07_Concepts/topics/smith_set.md) is all five candidates: the whole field is one big [cycle](../../01_Learn/cycle_resolution.md). When no one beats everyone, Ranked Robin falls back to the best overall record — and the best overall record can belong to someone who lost a match.
...
```bash
.venv/bin/python STARVote_LH_tabulation_engine/starvote_larry_hastings.py 05_Ranked_Robin/02_Examples/copeland_score/cases/copeland_half_credit_decides.yaml
```
...
*(Up: [05_Ranked_Robin](../../README.md) · concept docs: [Ranked Robin (RCV-RR)](../../01_Learn/README.md))*
