...
**Method:** [Ranked Robin (RCV-RR / Copeland)](../../../concepts) · **1 seat** · **Expected winner:** Ana
...
Winner — Ranked Robin (RCV-RR): Ana
   *** 3 candidates tie for the most wins (Ana, Bruno, Celia) — a Condorcet cycle (no candidate beats all others). Resolved by total margin, then lot order. (This is where Minimax / Ranked Pairs / Schulze differ — see 05_Ranked_Robin/concepts/cycle_resolution.md.)
```
...
```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py 05_Ranked_Robin/consensus_choice_divergence/cases/rr_vs_mwsl_cycle_c3_b32.yaml
```
