...
Thirteen voters, four candidates on a left→right line (Ada, Ben, Cara, Dan). Run it: [`ranked_robin_consensus_center.yaml`](../02_Examples/cases/ranked_robin_consensus_center.yaml) (or paste the `count:A>B>C` block into [LeGrand's calculator](https://www.cs.angelo.edu/~rlegrand/rbvote/calc.html)).
...
- **[Minimax / Simpson](../../07_Concepts/voting_paradoxes/minimax.md) is more decisive on any sizeable electorate**, precisely because it reads *margins* rather than win counts, and margins survive a cycle when win counts don't — [measured](../../07_Concepts/topics/ties/how_often_condorcet_methods_tie.md) at 7.9% against Copeland's 21.6% on 101 voters. **Not "strictly", though**, and the exception is worth knowing before quoting the claim at a committee: on an *even* electorate drawn matchups are margins of zero, minimax goes blind, and Copeland's half-a-win is the more decisive rule — on ten voters it ties 18.2% against minimax's 38.1%. The [minimal tilted cycle](../../method_comparisons/minimal_tilted_cycle/README.md) is the runnable proof: five voters, Copeland ties all three, minimax names a winner — and the information minimax used was on the ballots the whole time.
