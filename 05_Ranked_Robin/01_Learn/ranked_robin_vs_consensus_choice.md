...
*Third-party description of the other camp: [Better Choices for Democracy (electowiki)](https://electowiki.org/wiki/Better_Choices_for_Democracy) — records "Consensus Choice Voting" as the organisation's preferred term for **Condorcet methods**, with early work favouring **variants of Copeland's method**, and notes board member Wes Holliday, co-author of [Split Cycle](../../method_comparisons/split_cycle/). Useful because it names the algorithm that BC4D's own pages leave out — the same omission Equal Vote made until recently.*

*Worked, on real ballots: [Ranked Robin vs Consensus Choice — the same cycle, two different winners](../02_Examples/consensus_choice_divergence/) — 32 voters where the margins rule elects Ana and Most Wins, Smallest Loss elects Celia.*
