...
| **Pairwise / paired comparison** | the **family** (plain-descriptive) | general; both spellings in use | [Wikipedia](https://en.wikipedia.org/wiki/Round-robin_voting) |
| **Tournament voting** | the **family** (plain-descriptive, the sports metaphor made literal) | Wikipedia's round-robin article lists it alongside *paired comparison* | [Wikipedia](https://en.wikipedia.org/wiki/Round-robin_voting) |
| **Instant Round-Robin Voting** (**IRRV**) | **ambiguous — pin it down before relying on it.** electowiki defines a *specific* method (ranked ballots, approval ballots as tiebreak); the term is also used for **Condorcet methods generally**, and sometimes for **Schulze** | the [Condorcet Canada Initiative](https://condorcet.ca/) flies it as its banner tagline; Wikipedia redirects the term to *Round-robin voting* | [electowiki](https://electowiki.org/wiki/Instant-Round-Robin_Voting) |
...
| **Llull method** / **Llull voting** | the **same algorithm**, credited to its 1299 author instead of its 1951 one | rare in the wild, but not fringe: Wikipedia's Copeland article *leads* with "The Copeland or Llull method" | [Wikipedia](https://en.wikipedia.org/wiki/Copeland%27s_method) |
| **Ranked Robin** (**RCV-RR**) | a **branded method** — Copeland + a defined cycle tiebreak (sum of win margins) | [Equal Vote Coalition](https://equal.vote), 2021 — tiebreak proposed by [Sass](https://electowiki.org/wiki/User:Sass), name coined by [Sara Wolk](https://electowiki.org/wiki/Talk:Ranked_Robin) | [electowiki](https://electowiki.org/wiki/Ranked_Robin) · [equal.vote](https://www.equal.vote/ranked_robin) |
...
> **The same collision, one letter over: "IRRV."** *Instant Round-Robin Voting* repeats the Ranked Robin story almost beat for beat — a friendly tournament name, coined for advocacy, that some sources attach to **one method** and others to the **whole family**. electowiki gives it a specific definition (ranked ballots, approval ballots as the tiebreak); Wikipedia treats it as a redirect to the family; the Condorcet Canada Initiative uses it as a banner. So `IRRV` and `RR` are *both* names that shrink or swell depending on who is speaking — which is the argument for saying **Copeland** when you need to be exact, whichever brand your audience arrived with. *(electowiki here is a community wiki and reform-adjacent — right tier for what a brand name means, wrong tier for a verdict; condorcet.ca is an advocacy site, cite it for its own usage only.)*

## The *winner* has aliases too — and they are not the method's

A separate set of names describes the **candidate**, not the count, and they are interchangeable in a way the method names above are not:

**Condorcet winner** = **Condorcet candidate** = **pairwise champion** = **beats-all winner** — the candidate who beats every other head-to-head. All four are the same stipulated definition, and *none* of them names a method: Minimax, Schulze, Ranked Pairs and Copeland all elect this person whenever one exists. Reach for "beats-all winner" with a general audience — it is the only one of the four that explains itself — and note that it inherits the caution from [the "consensus" table below](#the-word-consensus-carries-three-different-jobs): it is a pairwise-majority fact, not a claim about warmth. → [Condorcet winner criterion (electowiki)](https://electowiki.org/wiki/Condorcet_winner_criterion).

**Three more aliases are in circulation — and unlike the four above, they are not safe to use interchangeably.** Wikipedia and reform sources increasingly gloss the Condorcet winner as a *majority winner*, a *majority-preferred candidate*, or a *tournament winner*. Each names the right candidate and imports a wrong idea:

| Alias | Names the right candidate, but… | Say instead |
|---|---|---|
| **majority-preferred candidate** | **Scope-ambiguous, and this repo uses it the other way.** Preferred by a majority *over every rival* (the whole field) is the Condorcet winner. Preferred by a majority *over one other candidate* is a single pairwise result — which is what [STAR's automatic runoff](../../01_STAR/01_Learn/the_count/STAR_Automatic_Runoff.md) delivers, and what the ~15 uses of "the majority-preferred **finalist**" across this library mean. The two are not the same, and STAR is **not** [Condorcet-compliant](#looks-like-condorcet-but-isnt-star-3-2-1). | **beats-all winner** for the field; **majority-preferred *finalist*** — keep the noun — for a runoff |
| **majority winner** | Collides with the ordinary meaning of *majority*: over half. A Condorcet winner may hold **no** absolute majority of first choices, and a candidate who *does* hold one is a different animal — the subject of the [majority criterion](../../07_Concepts/topics/majority_criterion/README.md), which STAR fails and Condorcet methods pass only incidentally. This library's [house rule on the word](../../07_Concepts/GLOSSARY.md#core-vocabulary) is to say **majority** only for over-half. | **beats-all winner**; reserve **majority winner** for >50% |
| **tournament winner** | "Tournament" already has a technical job here: the **C1** tier reads only the tournament graph, and a [tournament solution](../../07_Concepts/topics/tournament_solutions.md) usually returns a **set**, not a winner. The decoder's own table also files *tournament voting* as a name for the **family**. | **beats-all winner**; keep *tournament* for the graph and the C1 tier |

**The pattern worth noticing** is the same one this page flags for [the "round-robin voting" umbrella](#why-the-two-wikipedia-articles-overlap-the-trap-you-hit): the newer names are warmer and less mathematical than the ones they replace, and two of these three trade a precise pairwise fact for the word *majority*. That is a real gain in accessibility and a real loss in precision — worth making deliberately rather than by drift. Judge the swap on what the word does, not on who prefers it.

**One notation footnote, since it changes the arithmetic.** Modern Copeland scores a matchup **1 / ½ / 0** (win / tie / loss). Llull's original was **1 / 1 / 0** — a tie counted as a *win* for both. Same tournament, different standings, so a source using Llull's scoring can crown a different candidate on a ballot set with ties. Worth checking which one an implementation means before comparing results. Note also that the modern convention is not Copeland's invention and carries no theory with it: the English Football League had been ranking clubs on **2 / 1 / 0** — the same scoring doubled, so the same standings — since 1888, sixty-three years before Copeland's 1951 paper. Two things follow. It is worth having ready when someone calls the method academic or exotic: a Ranked Robin table is a league table, scored the way league tables have been scored for well over a century. And the League got there by [taking the vote we are still taking](https://en.wikipedia.org/wiki/1888%E2%80%9389_Football_League) — on 21 November 1888, mid-season, it **defeated 6–4** a West Bromwich Albion motion to award points for victories only. That is the same question as [what counts as a win](../03_Criteria/rr_tiebreaks/degrees_of_ties.md#the-rung-below-the-ladder-what-counts-as-a-win), which Ranked Robin's own spec leaves open and this repo records as unsettled; football settled it, for football, in 1888.
...
- **STAR** — add 0–5 scores, take the top two by total, then a pairwise automatic runoff. The runoff is pairwise, but the *finalists* are chosen by score sum, not by beating everyone — so STAR is **not** Condorcet-compliant. → [STAR](../../01_STAR/01_Learn/STAR_start_here.md) · [three notions of winner](../../01_STAR/01_Learn/properties_and_limits/STAR_three_winner_notions.md).
