...
|---|---|---|
| `allocated` | **[Allocated Score](01_Learn/STAR_PR/allocated_score.md)** — each winner "uses up" a quota of their strongest supporters. Equal Vote's recommended STAR-PR ([electowiki](https://electowiki.org/wiki/Allocated_Score)) | [02a — the count in full](02_Examples/cases/cases_pages/02a_c5_b63_proportional-allocated-score.md) |
| `sss` | **[Sequentially Spent Score](01_Learn/STAR_PR/sequentially_spent_score.md)** — each voter has a budget of "stars" that elected candidates spend ([electowiki](https://electowiki.org/wiki/Sequentially_Spent_Score)) | [02b — the count in full](02_Examples/cases/cases_pages/02b_c5_b63_proportional-sss.md) |
| `rrv` | **[Reweighted Range Voting](01_Learn/STAR_PR/reweighted_range_voting.md)** — ballots that already elected someone are down-weighted. The one that does **not** pass the [Hare Quota Criterion](01_Learn/what_proportional_means.md) ([electowiki](https://electowiki.org/wiki/Reweighted_Range_Voting)) | [02c — the count in full](02_Examples/cases/cases_pages/02c_c5_b63_proportional-rrv.md) |
...
- **[STAR-PR — the three methods](01_Learn/STAR_PR/README.md)** — quota + reweighting, and how `allocated` / `sss` / `rrv` differ. A page each: **[Allocated Score](01_Learn/STAR_PR/allocated_score.md)** (recommended) · **[SSS](01_Learn/STAR_PR/sequentially_spent_score.md)** · **[RRV](01_Learn/STAR_PR/reweighted_range_voting.md)** — mechanics, honest properties, and every scenario in this library gathered per method.
- **[When the STAR-PR methods disagree](02_Examples/method_divergences/README.md)** (301) — they usually elect the same slate, so it is easy to assume the choice is academic. Two elections of **three voters each** split all three apart, and you can check the arithmetic by hand.
- **The criteria, as runnable elections** (301) — the family's structural properties, each demonstrated in a handful of ballots: **[the Alabama paradox](03_Criteria/alabama_paradox/README.md)** (the committee grows by a seat and a sitting member loses hers — quota methods are not house-size monotone) · **[vote unitarity](03_Criteria/vote_unitarity/README.md)** (two bullet voters who spent nothing on the first winner keep their full star budgets, and those budgets decide the second seat).
- **[Two ways to fill a quota — the count-vs-weight engine divergence](03_Criteria/allocated_count_vs_weight/README.md)** (301) — an audit rather than a method property: upstream `starvote` fills allocation quotas by ballot *count* where the published definition (and BetterVoting) use ballot *weight*, and a three-slate fingerprint election seats different councils under the two accountings — 4-1-0 versus the proportional 3-1-1. Confirmed live against BetterVoting production, reported upstream, and **fixed in this library's vendored fork on 2026-08-09** — two library cases had carried the bug's fingerprints for months and were corrected with the patch.
