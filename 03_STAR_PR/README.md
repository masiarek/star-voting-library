...
- **[Two ways to fill a quota — the count-vs-weight engine divergence](03_Criteria/allocated_count_vs_weight/README.md)** (301) — an audit rather than a method property: this library's embedded `starvote` engine fills allocation quotas by ballot *count* where the published definition (and BetterVoting) use ballot *weight*, and a three-slate fingerprint election seats different councils under the two accountings — 4-1-0 versus the proportional 3-1-1. Confirmed live against BetterVoting production and reported upstream; two library cases carried the bug's fingerprints for months.
