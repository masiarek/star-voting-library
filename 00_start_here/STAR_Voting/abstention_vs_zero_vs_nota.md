...
- **Whole-ballot abstention** — every line blank (or, in STAR, *all lines equal*, e.g. `0,0,0` or `5,5,5`). The ballot expresses **no preference**, so it's set aside as an **abstention**: it counts as turnout but contributes nothing to any candidate's score or to the runoff.
