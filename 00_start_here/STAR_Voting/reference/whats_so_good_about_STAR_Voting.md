...
- **[SLIDE]** — show a slide from one of the decks (see [`conversation_scripts.md`](../../about_this_repo/conversation_scripts.md) for the map).
...
> [SLIDE] Full Deck — "Why STAR Voting?" title + the one-line pitch. [REPO] [`00_START_HERE.md`](../../00_START_HERE.md) (the one-sentence pitch).
...
> [DEMO] `method_comparisons/split_voting/_main/01_political_left_split.yaml` — a 66% coalition (Green, Labour, SocialDem) splits three ways; under Choose-One the **Conservative wins with 34%**, a candidate two-thirds ranked last. The `[Vote-splitting check]` block says it in numbers. Then point out: *STAR elects Labour* — the coalition's consensus. [SLIDE] Full Deck — "PROBLEM: VOTE-SPLITTING" and "VOTE-SPLITTING SETS OFF A DOMINO EFFECT."
...
> [REPO] The deeper *diagnosis* — is the system really broken, how often, how serious? — [`our_voting_system_is_broken.md`](../../topics/our_voting_system_is_broken.md) (the foundational "Problem with Plurality" episode).
...
> [SLIDE] Full Deck — "INTRODUCING: ALTERNATIVE VOTING! (Rating vs Ranking)" and "That changed with the invention of STAR Voting… Score + Instant Runoff." [REPO] [`GLOSSARY.md`](../../GLOSSARY.md) — "Scored (cardinal)" vs "Ranked (ordinal)."
...
> [DEMO] `01_STAR/silly_two_cand_STAR/01a_c2_b1_two-candidates.yaml` — one voter, two flavors. Watch the Scoring Round, then the Automatic Runoff. With two candidates STAR agrees with ordinary voting — that's *why* we start here: the gears are visible, nothing surprising competes for attention. [DEMO] Then `01_STAR/_main/vote_splitting2.yaml` (or `method_comparisons/split_voting/_main/02…`) — add a third candidate and watch the winner become the broad compromise. [SLIDE] Full Deck — "HOW DOES STAR VOTING WORK?" and "STAR RESULTS: 1) Add up the stars. 2) Add up the votes." [REPO] [`00_START_HERE.md`](../../00_START_HERE.md) — "Why TWO rounds and not just scores?"
...
> [SLIDE] Full Deck — "WHAT DO WE WANT IN A VOTING METHOD?", "FAIR-ACCURATE-EQUAL", the accuracy chart ("YOU ARE HERE / YOU COULD BE HERE"), and the summability slides. [REPO] [`Why_STAR_Voting.md`](../../topics/Why_STAR_Voting.md) — Slides 5–9 bullets.
...
> [SLIDE] Full Deck — "RCV COMMON FALSE CLAIMS" table, "Burlington 2009", "Alaska '22 Special", "Single-Winner Voting Method Scorecard" (Torrance deck). [REPO] [`Why_STAR_Voting.md`](../../topics/Why_STAR_Voting.md) Part 2 — Tier 1 #2/#3 and Tier 3 #9 (later-no-harm / center squeeze) for the rebuttals. [REPO] The scorecard's "Spoiler / vote-splitting? No" is the *forced* kind; the narrow self-inflicted residual is [`residual_vote_splitting.md`](../properties_and_limits/residual_vote_splitting.md) (demos `method_comparisons/split_voting/_main/05a`–`05b`). [DEMO] Optional: run a strict 4-candidate file through both engines and show the `[Divergence from STAR]` block where RCV-IRV and STAR disagree. <!-- terminology-ok: RCV in slide/deck titles -->
...
| Beat | File | Shows |
|------|------|-------|
| Spoiler | `method_comparisons/split_voting/_main/01_political_left_split.yaml` | majority split → minority wins Choose-One; STAR fixes |
| Spoiler (food) | `method_comparisons/split_voting/_main/03_lunch_veggie_vs_meat.yaml` | same effect, lighter theme |
| Two rounds | `01_STAR/silly_two_cand_STAR/01a_c2_b1_two-candidates.yaml` | gears visible, no surprise |
| Compromise | `01_STAR/_main/vote_splitting2.yaml` | 3rd candidate → consensus winner |
...
See [`conversation_scripts.md`](../../about_this_repo/conversation_scripts.md) for the full series plan, slide map, sync strategy, and demo-software recommendation.
