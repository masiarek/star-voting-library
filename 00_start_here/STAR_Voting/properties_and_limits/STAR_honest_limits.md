...
There is a known way to shrink this particular problem, worth naming because it changes the shape of the objection rather than just conceding it. Cryptographic voting systems replace *"please proofread your printout"* with a **cast-or-challenge** step (the *Benaloh challenge*): once the machine has committed to an encrypted ballot, the voter may either cast it **or** spoil it and force the machine to prove it encrypted the right thing. Because the machine can't tell in advance which the voter will do, cheating is caught with high probability once even a small fraction of voters challenge — and the voter's check becomes a **short code match** rather than a reading task, which is exactly the failure the research above identifies. This is [ElectionGuard](https://github.com/Election-Tech-Initiative/electionguard)'s approach, and it composes naturally with STAR because the tally is [summable](../../topics/summability/). It's a mitigation, not a cure — it's currently rare in practice — but it means "scored ballots are harder to verify" is an argument about *today's* equipment, not a permanent property of the method.
