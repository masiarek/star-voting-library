...
**Accurate.** Research comparing **ratings and rankings** finds ratings have superior validity — forced full rankings capture *noise*, differences voters don't actually feel. (*"Ratings"* is the measurement literature's term for what STAR calls **scores** — a 0–5 rating *is* a score; we keep the research word here because that's how the studies name it.) Equal scores let voters express exactly the distinctions that matter to them and no more. (The deep dive: [Scores vs. Ranks](../scores_and_ranks/scores_vs_ranks.md).)
