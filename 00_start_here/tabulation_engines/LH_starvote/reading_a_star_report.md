...
**2. [Divergence from STAR].** A quick cross-check against other methods. `STAR = A` but `Approval = C` — i.e., a pure *score/approval* count (most total stars) would pick **C**. When this line shows a difference, you're looking at a case where the scoring total and the STAR winner disagree — the signal of a **Runoff Reversal**. One honesty rule: when a comparison method's top spot is an *exact tie*, the block prints it as one — `Choose-One (Plurality) = tie (Ann / Bob)` — instead of silently breaking the tie to a single name. A tie counts as "(differs from STAR)" only when the STAR winner is **not** among the tied candidates; a tie that includes the STAR winner is labeled `(tie — includes the STAR winner)` and doesn't count as a divergence.
