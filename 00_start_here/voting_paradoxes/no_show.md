...
Score-summing methods (Score, Approval) cannot produce a no-show paradox — an added sincere ballot only adds support in the direction the voter intends. STAR's runoff stage costs it the formal guarantee, and Condorcet methods provably fail participation in some electorates (Moulin's theorem) — but on *this* pair both STAR and Ranked Robin elect Beth with 11 voters and with 9, so nobody here was ever punished for showing up. See the honest-limits framing in [reading_these_fairly.md](../../method_comparisons/paradoxes_and_whoops/reading_these_fairly.md).
