...
**Analytically sound, and it generalizes past his own proposal.** A Top-N runoff is only as good as the primary that fills the N slots: a [Choose-One](../plurality.md) primary re-imports [vote-splitting and the spoiler effect](../spoiler_effect.md) at the qualifying stage, and can eliminate a broadly-liked candidate *before* the clever general ever sees them — the same failure that makes [California's Top-2](../two_party_dominance.md) shut out compromise candidates. Using [Approval](../../../04_Approval/concepts/approval_voting.md) (already live as "Approval Top-2" in **St. Louis**) to seed the finalists is a defensible fix. The repo-flavored footnote: **STAR needs no separate primary at all** — its 0–5 ballot does the [semifinal (top-two) and the final runoff in one pass](../../STAR_Voting/the_count/STAR_Automatic_Runoff.md), which is a different answer to the same "don't let a bad primary spoil a good final" concern.
