...
→ Spoken version: [What Is a Voting Method? (conversation)](what_is_a_voting_method.md) · primer: [Ballot & terminology basics](ballot_and_terminology_basics.md) · next: [STAR — start here](../../01_STAR/concepts/STAR_start_here.md).
...
A majority rule wouldn't accept that. Since nobody cleared half, it demands another step: a runoff. STAR's [automatic runoff](../../01_STAR/concepts/the_count/STAR_Automatic_Runoff.md) is that step — after scoring, the top two meet head-to-head, and the winner holds a majority *between those two finalists*. That's not proof the winner is everyone's favorite across the whole field, but it's exactly the check plurality never even performs.
...
- The method this library teaches: [STAR — start here](../../01_STAR/concepts/STAR_start_here.md) · [its benefits](../../01_STAR/concepts/getting_started/STAR_benefits.md)
