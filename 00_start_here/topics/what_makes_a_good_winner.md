...
**VSE has an academic sibling worth knowing about: [distortion](distortion.md).** Same premise — voter utility is what an election is trying to find, and a ballot is a lossy channel — but proved instead of simulated, and worst-case instead of average-case. That matters when someone waves VSE away as reform-movement math: the peer-reviewed computer-science literature (AAAI/IJCAI/FOCS, fifteen-plus years) made the *identical* modeling choice and derived bounds. It also supplies results the simulations can't: Copeland/[Ranked Robin](../RCV_Ranked_Robin/ranked_robin.md) is within a constant factor of the welfare optimum however large the field, while STV/IRV's bound grows with it — and a ranking plus *a few* intensity questions provably collapses the worst case from quadratic to constant.

VSE is an **average-case** score; its worst-case counterpart is **[metric distortion](metric_distortion.md)** — the theorem that even purely ordinal rules, on spatial electorates, elect within a provable 3× of the utilitarian optimum. Read together: distortion is the guarantee, VSE the expectation.
