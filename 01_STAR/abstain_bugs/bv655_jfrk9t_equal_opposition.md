...
2. **The record keeps them separate in LH.** LH tabulates both an explicit `0` and an abstention marker (`~`/`&`) as 0, but stores them distinctly — so a report can tell "rejected everyone" from "didn't vote." BetterVoting stores only `0`/`null` and has no explicit abstain mark, so it can't (see the [abstain issue index](../../07_Concepts/tabulation_engines/BV/abstain_issues_index.md) and the [lesson](../concepts/properties_and_limits/abstention_vs_zero_vs_nota.md)).
