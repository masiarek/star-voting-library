...
1. **Create a STAR election on BetterVoting** — 2 candidates, e.g. `A` and `B` (or `Dog`/`Cat` to match the big case).
...
3. **Read BetterVoting's result** and note `nAbstentions` / `nTallyVotes` (Race Details / export). **Hypothesis:** BV reports **3 abstentions** (the 5,5 + 0,0 + blank), i.e. it flags the engaged `5,5` voter as abstaining, and tallies 3.
4. **Export the JSON**, drop it in this folder as `small_abstention_c2_b6_bv_export.json`, and **convert it to YAML**:

   ```
   python YAML_library/1_positive/01_convert_json_yaml.py <exported.json>
   ```

5. **Tabulate the YAML with the LH engine** and confirm the *correct* reading (6 cast, 1 abstention, 3 Equal Support, A wins 2–1) — i.e. it matches the synthetic file above. The contrast between the two reports is the lesson.
6. **Freeze + document:** add the BV numbers to a snapshot (like [BetterVoting result — frozen snapshot (pet race)](BV_result_snapshot.md)) and cross-link from the lessons. Update the GitHub issue with the real small-case screenshot.
