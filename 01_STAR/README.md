...
4. **The headline lesson — top scorer ≠ winner.** STAR's single most important behavior, as a 3→9-candidate progression with a control case → **[Runoff Reversal](02_Examples/runoff_overturns_leader/)**.
5. **The spoiler, in numbers.** Vote-splitting and the compromise winner → [the vote-splitting scenarios](02_Examples/) · cross-method: [the split-voting set](../method_comparisons/split_voting/).
...
9. **The Majority Criterion** (and the "relaxed" version) → [the two 5-voter elections](03_Criteria/majority_criterion/).
10. **The "dead rung"** — when STAR's five-star tiebreaker can't fire and the tie falls to the lot → [the dead-rung case](03_Criteria/tie_break_dead_rung/).
11. **None of the Above** — a protest electorate where NOTA actually wins → [the NOTA election](03_Criteria/none_of_the_above/).
...
Five numbered buckets, in reading order. **Every method folder in the library uses the same five**, so once you know the shape here you know it for [Bloc STAR](../02_STAR_Bloc/), [Proportional STAR](../03_STAR_PR/), [Approval](../04_Approval/) and [Ranked Robin](../05_Ranked_Robin/) too. Difficulty is *not* in the folder names — a case can be 101 for its basic idea and 301 for the deep dive — so levels are marked per set, and the authoritative sequence stays in the [curriculum](../07_Concepts/CURRICULUM.md).
...
| [IIA & the cycle spoiler](03_Criteria/iia_cycle_spoiler/) | 03 | 301 | In a genuine Condorcet cycle, a candidate who cannot win still changes who does — on sincere ballots. |
| [The dead rung — when the tiebreak can't fire](03_Criteria/tie_break_dead_rung/) | 03 | 301 | The "dead rung": when no tied candidate holds a 5, the five-star tiebreak reads 0–0 and the lot decides. |
| [None of the Above](03_Criteria/none_of_the_above/) | 03 | 301 | BV215 — a protest election where None of the Above tops the scores and wins the runoff. |
| ["What Makes the Best Pet?"](04_Real_Elections/pet_real_bv_election/) | 04 | 201 | A real 461-ballot BV race imported and reconciled line by line — the whole pipeline, raw ballots to winner. |
| [Runoff reversals on BV](04_Real_Elections/runoff_reversal_bv_cases/) | 04 | 201 | The headline behavior on real ballots: BV's own screenshots beside the engine's report. |
