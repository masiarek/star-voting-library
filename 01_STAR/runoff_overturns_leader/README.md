...
Concept hub for all of it: [The Automatic Runoff Round](../concepts/the_count/STAR_Automatic_Runoff.md).
...
The last two are the contrasting pair behind the [second-round FAQ](../concepts/the_count/STAR_second_round_FAQ.md) — one reversal almost everyone accepts, one that's a genuine philosophical trade-off (a token no-hoper keeps each a proper three-way race).
...
```
uv run python STARVote_LH_tabulation_engine/starvote_larry_hastings.py "01_STAR/runoff_overturns_leader/cases/03_c7_b3_ice-cream-live.yaml"
```
