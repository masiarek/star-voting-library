...
Everything in one file: the [`_tabulated` mirror](../runoff_reversal_bv_cases_tabulated/Runoff_06_confirms_at_scale_d664xw_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py 01_STAR/runoff_reversal_bv_cases/Runoff_06_confirms_at_scale_d664xw.yaml
```
