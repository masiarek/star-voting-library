...
1. **Matches won** — favor the candidate who wins the most head-to-head matchups (this is the **[Copeland](../../../05_Ranked_Robin/01_Learn/README.md)** score, and it is the "Condorcet" in the name: a candidate who beats every other *tied* candidate head-to-head wins here immediately).
...
If this ladder feels like it belongs to a different method, that instinct is correct. "Most head-to-head matchups won, break ties by margin" **is** the [Ranked Robin](../../../05_Ranked_Robin/01_Learn/README.md) (Copeland) algorithm. Equal Vote's Condorcet Tiebreaker is Ranked Robin's own logic **borrowed to settle a STAR tie** — a miniature round-robin run *only among the tied candidates*, and *only* when STAR's normal rounds (score, then head-to-head) finish exactly even.
...
So Ranked Robin is where this arithmetic *lives as a whole method*; here the same arithmetic is a *subroutine* that only fires on an exact STAR tie. (Ranked Robin's own tie-break story — Copeland ties broken by margin, and where LH and BetterVoting diverge — is [rr_tiebreak_lh_vs_bv.md](../../../05_Ranked_Robin/01_Learn/rr_tiebreak_lh_vs_bv.md).)
...
- [The cycle-spoiler case (BV2212)](../../03_Criteria/iia_cycle_spoiler/bv2212_g3f7r2_cycle_spoiler.md) — a Condorcet cycle driving a STAR result with no tie at all.
