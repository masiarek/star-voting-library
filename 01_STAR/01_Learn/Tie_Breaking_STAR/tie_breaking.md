...
- **[Equal Vote's optional *Condorcet Tiebreaker*](condorcet_tiebreaker.md)** (a **third** protocol — an alternative to the recommended 5-Star Tiebreaker for *hand counts*: matches-won → total preference votes → win margin → random). It is a **tiebreaker, not Condorcet compliance** — it only fires on an actual tie, and a candidate who never reaches the runoff can't be rescued by it, so STAR stays not Condorcet-compliant. Its rung 2 (preference votes / margin) differs from LH's (five-star), so on a genuine tie it can pick a different winner than either LH or BetterVoting. Worked on two examples, with the compliance-vs-tiebreaker distinction, on that page (source: <https://www.starvoting.org/condorcet_tiebreaker>).
- **Every method's ladders, engine by engine:** [Tiebreak ladders](../../../07_Concepts/tabulation_engines/tiebreak_ladders.md) — this page's two STAR ladders in one table beside Ranked Robin, RCV-IRV, Approval, Plurality, and the PR family, across every engine this library reaches.
- Glossary: **Tiebreaker**, **Head-to-head / pairwise**, **Automatic runoff**, **Equal Support** — [Glossary — voting methods & criteria](../../../07_Concepts/GLOSSARY.md)
- Equal-score handling in the runoff: ["Aren't Equal-Score Votes Just Discounted?"](../reference/are_equal_score_votes_discounted.md) · demo [`equal_support_runoff_demo.yaml`](../../02_Examples/cases/equal_support_runoff_demo.yaml)
