...
**And the sweet spot isn't only a voting-reform hunch.** Wherever judgments get collected and averaged at scale — school and university grading, customer quality ratings, political opinion scales like the German *Politbarometer* — the discrete scale that gets chosen lands on roughly **five or six values**, which Hillinger reads as a practical ceiling on how finely people discriminate along a cardinal scale ([the full roster, and where the figure comes from](../../../07_Concepts/topics/cardinal_utility.md#where-the-5-or-6-levels-figure-comes-from)). Three independent domains converging on the same resolution is a better argument for 0–5 than familiarity alone. Worth knowing which way it cuts, though: the same section records that Hillinger himself would hand a *general electorate* a **three-level** ballot and reserve five levels for expert committees — so the convergence supports "about this many levels," not "exactly six."
