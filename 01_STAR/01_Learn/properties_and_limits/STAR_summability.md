...
The mirror of [IRV's non-summable example](../../../06_Other/RCV_IRV/concepts/RCV_IRV_lack_of_summability.md#worked-example-two-districts-both-won-by-b-merged-b-loses). Same setup, opposite result — run [`summability_demo/`](../../../method_comparisons/summability_demo/README.md) (three trees: Maple, Oak, Pine). The two districts even have *different* winners (Maple in A, Oak in B — district B is a runoff reversal), so the merge is genuinely non-trivial:
