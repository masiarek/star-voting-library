...
Because a blank means "I did not support this candidate," which is exactly what a 0 encodes — it can't help that candidate advance. But STAR's engine keeps *why* a zero is a zero: this library records a small **marker vocabulary** — `-` blank · `~` race abstention · `&` candidate abstention · `?` spoiled · `%` spoiled+reissued — all of which tabulate as 0 but are reported distinctly, so a deliberate abstention isn't confused with an active low score. A *whole* ballot left blank is set aside as an abstention — but **set aside is not thrown out**: it still counts as **turnout** (and toward quorum), it just carries no preference to fold into any candidate's total. An all-*equal* scored ballot (`0,0,0`, `5,5,5`) is a bookkeeping fork between the two tabulators: **BetterVoting** sets it aside as an abstention too, while the **LH engine** keeps it cast — its stars tally in the scoring round and it files under Equal Support in the runoff. Neither choice can change the winner: an all-equal ballot shifts every candidate by the identical amount, so it tips no contest either way. Full detail: [Abstention vs. a zero vs. NOTA](../properties_and_limits/abstention_vs_zero_vs_nota.md) and ["Preference" — the word that causes half the confusion](../../../07_Concepts/topics/preference.md).
