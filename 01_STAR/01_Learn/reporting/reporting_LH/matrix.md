...
`A > B | 2 - 2 - 1`: 2 prefer A, 2 Equal Support, 1 prefers B — the same 2 vs 1 that decides the runoff (`Voters with a preference: 3 of 5 (2 Equal Support)`). Nothing new, hence hidden by default. (→ [Runoff percentages](../../the_count/runoff_percentages.md).)
...
These are **engine defaults**, not something you write into a case file — case files carry no `options:` block (dropped repo-wide 2026-08-09). To see more than the default render, read the case's `_tabulated.txt` mirror or add `--full` to the run. Details: [**LH reporting options**](options.md).

- **`show_matrix`** — print the matrix at all. Default `true`, auto-suppressed for two-candidate races (it would just echo the runoff) and for multi-winner (a "Top 2 Finalist" matrix is a single-winner idea).
- **`matrix_finalists_only`** — default `true` on screen: only the two finalists' row/column, a compact check that the runoff matches the pairwise result. The saved `_tabulated.txt` and `--full` force the **full** N×N grid.
...
**One caution about the multi-winner suppression.** The auto-gate only covers the *screen*: a Bloc/PR case's `_tabulated.txt` (and any `--full` run) still renders the grid, still titles it *Runoff (Preference) Matrix*, and still stars two candidates as "Top 2 Finalist" — those stars come from the engine's **single-winner** STAR analysis of the same ballots, and have nothing to do with which candidates won seats. Read that grid as plain head-to-head data.
