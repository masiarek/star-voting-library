...
→ Hub: [STAR Reporting](README.md) · [How the LH engine reports](reporting_LH/) · [How BetterVoting reports](reporting_BV/) · imported tie data: [tie-break JSON](../Tie_Breaking_STAR/tie_breaking_JSON.md).
...
— here broken by the **Scoring Round** (Vanilla had the higher total), exactly the ladder's first rung. (Worked end-to-end, with ties in *both* rounds: [Tie-Breaking — the Ice Cream example](../Tie_Breaking_STAR/tie_breaking.md#worked-example-ice-cream-6-flavors-ties-in-both-rounds).)
...
<!-- Screenshot slot — LH engine tie report.
     Source: run election `02b_c3_b2_three-candidates` (in `01_STAR/02_Examples/cases/`) and
...
BetterVoting records how a tie was settled in its result data as **`tieBreakType`**, and shows the resolved order on the result card. (For how an imported BetterVoting tie-break maps into a YAML election's lot order, see [tie-break JSON](../Tie_Breaking_STAR/tie_breaking_JSON.md).)
