...
Full report from the [`_tabulated` mirror](../cases_tabulated/ex09_game_night_cycle_tabulated.txt) (regenerated on every run; every analysis forced on):
...
Winner — Ranked Robin (RCV-RR): Azul
   *** 3 candidates tie for the top Copeland score, 2 (Azul, Boggle, Catan) — a Condorcet cycle (Azul → Boggle → Catan → Azul: no candidate beats all others). Resolved by total margin, then lot order. (This is where Minimax / Ranked Pairs / Schulze differ — see 00_start_here/RCV_Ranked_Robin/cycle_resolution.md.)
```
