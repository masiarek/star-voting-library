...
Method-specific terms for **STAR** (Score Then Automatic Runoff). Shared, cross-method vocabulary (monotonicity, Condorcet, summability, the Equal Vote criterion, spoiler / vote-splitting…) lives in the [main glossary](../../../00_start_here/GLOSSARY.md) — this page only defines what's specific to STAR.
...
- **Scoring round** — STAR's first round: add up each candidate's scores; the top two become finalists. → concept [The Scoring Round](../the_count/STAR_Scoring_Round.md); demo [`equal_support_runoff_demo`](../../_main/cases/cases_pages/equal_support_runoff_demo.md)
...
- **Equal Support** — scoring the two finalists the same; counts as "no preference" between them in the runoff (still fully counted in the scoring round). The aka is documented once in the [main glossary](../../../00_start_here/GLOSSARY.md). → ["Aren't Equal-Score Votes Just Discounted?"](are_equal_score_votes_discounted.md)
- **Runoff Reversal** — the Scoring-Round leader **loses** the Automatic Runoff to the finalist more voters prefer (the *score* winner ≠ the *STAR* winner). Not a malfunction — the runoff elects the finalist preferred by the majority (of voters with a preference); the engine flags it with a `[Runoff Reversal]` block. House term: "the runoff overturns the score leader." → walkthrough [Runoff Reversal](../../runoff_overturns_leader/)
...
- **Dead rung** — the five-star rung when no tied candidate scored a 5 (e.g. all scores capped at 4): it reads `0–0`, separates nobody, and the tie **falls through to the lot**. Equal *non-zero* five-star counts do the same. Mnemonics: *"no fives, no rung — drop to the lot"* / *"it counts fives, not fours."* akas: broken / missing / empty / phantom rung. → cases [The "dead rung"](../../tie_break_dead_rung/) · generator [`generate_dead_rung_scenarios.py`](../../../STARVote_LH_tabulation_engine/tools_adam/generate_dead_rung_scenarios.md)
- **Quorum** — an opt-in *turnout* threshold (`eligible_voters` + `quorum`): enough of the eligible electorate must participate or no winner is declared. → [Quorum — did enough of the electorate show up?](../../../00_start_here/topics/quorum.md)
...
*Shared criteria STAR is judged on — [monotonicity, Condorcet efficiency, summability, later-no-harm, the Equal Vote criterion](../../../00_start_here/GLOSSARY.md#properties-criteria) — are in the [main glossary](../../../00_start_here/GLOSSARY.md).*
