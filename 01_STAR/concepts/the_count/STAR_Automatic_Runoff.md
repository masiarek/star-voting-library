...
→ Round 1: [The Scoring Round](STAR_Scoring_Round.md) · the whole report, section by section: [Reading a STAR report](../../../00_start_here/tabulation_engines/LH_starvote/reading_a_star_report.md) · the two-step count vs IRV: [Tabulation, step by step](../../../00_start_here/topics/tabulation_star_vs_irv.md) · how it's displayed: [STAR Reporting](../reporting/).
...
It also keeps the ballot **honest**. Without a runoff, the smart way to vote in a pure scoring election is to give your favorite a 5 and everyone else a 0 — any in-between score only helps a rival catch up. If everyone does that, the 0–5 ballot collapses to just 0s and 5s, which is **[Approval voting](../../../00_start_here/GLOSSARY.md)**: strategic score voting *degenerates into Approval*, and the expressive scale stops meaning anything. STAR's runoff removes that incentive — because it only asks which finalist you scored higher, your honest 3s and 4s still do real work, so you never have to exaggerate. **The runoff is what lets STAR keep an expressive ballot people can use honestly.**
...
A **Runoff Reversal** is just *how much* and *how many* pointing at different candidates: the leader had more total support, but fewer voters prefer them head-to-head. It's the safeguard working, not a glitch. Worked examples, the voter patterns behind it, and how to teach it: [Runoff Reversal](../../runoff_overturns_leader/).
...
(From [`flat_scores_abstention_c3_b8`](../../pet_real_bv_election/cases/flat_scores_abstention_c3_b8.yaml).)
...
|---|---|---|
| Runoff Reversal | score leader loses the runoff | [When the top-scoring candidate isn't the winner](../../runoff_overturns_leader/) |
