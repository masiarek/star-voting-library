...
**Run it:** [the equal & opposite case](../../equal_and_opposite/) makes this tabulatable — a base election with a decisive winner, then the *same* election with two mirror ballots added, and STAR elects the same candidate either way (every score total just rises by 5, and the runoff cancels 1–1).
...
| **RCV-IRV** | ❌ fails — opposite ballots don't reliably cancel under sequential elimination ([why](../../../06_Other/RCV_IRV/concepts/RCV_IRV_equal_vote.md)) |
...
**The other side, fairly stated:** RCV-IRV advocates (e.g. FairVote) argue RCV-IRV upholds one-person-one-vote better than Choose-One. That debate — and why the equal-vote camp says RCV-IRV still fails the balance test in competitive races — is on the companion page, [RCV-IRV fails the Equal Vote](../../../06_Other/RCV_IRV/concepts/RCV_IRV_equal_vote.md).
...
For **multi-winner** elections the equal-vote idea extends to **Vote Unitarity** (Keith Edmonds) — an equal vote *plus* proportionate spending — which motivates proportional score methods like **Sequentially Spent Score (SSS)**. See [proportional: STV vs STAR-PR](../../../03_STAR_PR/concepts/stv/proportional_stv_vs_star.md) for that thread.
