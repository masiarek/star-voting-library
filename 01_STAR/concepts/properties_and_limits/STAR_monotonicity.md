...
→ The method that *fails* this is RCV-IRV — see [`IRV non-monotonicity`](../../../06_Other/RCV_IRV/concepts/RCV_IRV_non_monotonicity.md) (where more first-place votes can defeat the winner). Glossary: [`monotonicity`](../../../00_start_here/GLOSSARY.md).
...
→ [`monotonicity_star_before.yaml`](../../../method_comparisons/monotonicity/cases/monotonicity_star_before.yaml) · [`monotonicity_star_after.yaml`](../../../method_comparisons/monotonicity/cases/monotonicity_star_after.yaml)
...
**See it, runnable:** a worked before/after pair where raising X and deleting the loser below X flips the winner from X to Z — [`mono_raise_delete_before.yaml`](../../../method_comparisons/monotonicity/cases/mono_raise_delete_before.yaml) (X wins) · [`mono_raise_delete_after.yaml`](../../../method_comparisons/monotonicity/cases/mono_raise_delete_after.yaml) (X raised, Y deleted → **Z** wins). The only change is nine ballots going `Z=5,X=3,Y=2` → `Z=5,X=4,Y=0`; burying Y drops it below Z, so the runoff becomes X vs Z, which X loses.
