...
Austen + Camus.
NOTE — which Droop quota: that walkthrough is the HAND COUNT's,
floor(9/3)+1 = 4, and the report below runs the other standard Droop
quota, the exact 9/3 = 3.00, so its intermediate numbers differ —
Austen's surplus is 2 (each ballot moving at 0.4), Bronte finishes
level with Camus at 3.00, and no elimination round runs at all. Both
rules are standard, both seat Austen + Camus; part (f) of
ex14_transfer_machine.md tables the two counts side by side.
