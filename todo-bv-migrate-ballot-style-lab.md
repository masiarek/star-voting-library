...
- **Freeze UI exports** when convenient: `7mckyg`, `b6xrdr` (FBC pair) · `7q6by8`, `fxhw6g` (burial pair) · `fvg8y8` (food-truck row) · `39py93` (STV control). The crashers `gvtg2h`/`8xwx43` export without Results until BV ships the fix — re-export then. *(Verified still ALL missing 2026-07-23 — the case folders have only yamls + mirrors; the trio rule is broken for all eight until the exports land.)*
