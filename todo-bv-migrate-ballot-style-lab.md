...
- **Missing folder `README.md` landing pages** (the site renders these sections as bare file lists): `00_start_here/{about_this_repo, curriculum, tabulation_engines, tips, other_ranked_methods}`, `01_STAR/concepts/{getting_started, the_count, properties_and_limits, reference}`, `03_STAR_PR/concepts/stv/`, and `method_comparisons/split_voting/_main/` (the only `_main` in method_comparisons without one).
