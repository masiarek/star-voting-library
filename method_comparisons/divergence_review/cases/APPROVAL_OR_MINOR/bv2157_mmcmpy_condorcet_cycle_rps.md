...
Winner — Ranked Robin (RCV-RR): Rock
   *** 3 candidates tie for the top Copeland score, 1 (Rock, Paper, Scissors) — a Condorcet cycle (Rock → Paper → Scissors → Rock: no candidate beats all others). Resolved by total margin, then lot order. (This is where Minimax / Ranked Pairs / Schulze differ — see 00_start_here/RCV_Ranked_Robin/cycle_resolution.md.)
```
