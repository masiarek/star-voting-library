...
STAR, RCV-IRV and Ranked Robin all agree on **Carmen**. Only **Approval** reports **Andre** — but that line is a **conversion artifact**, not an Approval verdict: the comparison converts score ballots with a 3+ stars approval threshold, and on this profile 2 candidates tie for the most approvals (40 each) — the printed winner is just the candidate-priority tie-break. A caution about applying a fixed approval cut to low-scoring ballots, not an Approval-vs-STAR difference.
