...
On these ballots RCV-IRV reports **Amos** rather than STAR's **Cole**, but this is an **artifact of score→rank conversion**, not a robust method difference: 9 of 9 ballots have tied non-zero scores, so their ranks are set by an arbitrary candidate-priority order; the RCV-IRV winner flips when that priority order is reversed. Ranked Robin and the Condorcet (pairwise) winner both agree with STAR (**Cole**). **Do not use this case to criticize RCV-IRV** — the disagreement is a coin-flip created by equal scores and would vanish under a different (equally arbitrary) tie-break.
...
the honest answer to "who is even in contention?".
   Smith set (1 of 3): Cole
   Outside (2):        Amos, Bree
   One member ⇒ Cole is the Condorcet winner, beating every rival head-to-head.
   RCV-IRV winner Amos is OUTSIDE the Smith set. ✗
      Every member of the set (Cole) beats Amos head-to-head, yet
      RCV-IRV elected Amos anyway. RCV-IRV is not Smith-efficient (nor
...
the honest answer to "who is even in contention?".
   ...
   Outside (2):        Amos, Bree
   One member ⇒ Cole is the Condorcet winner, beating every rival head-to-head.
   Ranked Robin (RCV-RR) winner Cole is INSIDE the Smith set. ✓
