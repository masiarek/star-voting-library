...
- Scanned **238** single-winner STAR elections (skipped 201 non-eligible files: multi-winner / Approval / RR / RCV / ranked-ballot / unparseable).
- **142** (60%) diverge from STAR under at least one method; **96** agree across the board.
...
Review order is the teaching value of each bucket. Each case links to a full teaching `.md` (ballots + every method's report + a plain-English explanation) under `cases/`. Listing is **deduped** to one entry per distinct election (139 cases; identical library copies merged).

**79 of these 139 elections are live on BetterVoting** and carry a `BV ↗` link straight to the results page — so the divergence can be checked against a tabulator nobody in this repo wrote, not just re-read from our own output. Each case page repeats the link under its title.
