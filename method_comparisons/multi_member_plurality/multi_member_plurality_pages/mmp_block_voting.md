...
Full report from the [`_tabulated` mirror](../multi_member_plurality_tabulated/mmp_block_voting_tabulated.txt) (regenerated on every run; every analysis forced on):
...
```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/multi_member_plurality/mmp_block_voting.yaml
```
