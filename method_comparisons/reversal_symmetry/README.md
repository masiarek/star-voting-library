...
→ Related: [IRV non-monotonicity](../../06_Other/RCV_IRV/concepts/RCV_IRV_non_monotonicity.md) · [the participation / no-show paradox](../participation_no_show/README.md) · [Condorcet cycles](../../05_Ranked_Robin/01_Learn/cycle_resolution.md) · glossary [`reversal symmetry`](../../07_Concepts/GLOSSARY.md).
...
- **The failure is real and IRV-specific** — worth knowing, and a clean symptom of the same root cause as [center squeeze](../../07_Concepts/topics/center_squeeze/README.md) and [non-monotonicity](../../06_Other/RCV_IRV/concepts/RCV_IRV_non_monotonicity.md): IRV eliminates on *first-choice counts*, so reversing preferences doesn't cleanly reverse the outcome.
...
- **Two of the source's other "disasters" are things we already cover.** The page's "strategic reversal" twist (raising A from bottom to top makes A *lose*) is plain [non-monotonicity](../../06_Other/RCV_IRV/concepts/RCV_IRV_non_monotonicity.md); its "no-show" twist is the [participation paradox](../participation_no_show/README.md). Nothing new there.
- **Disclose the lean.** The source is [rangevoting.org](https://www.rangevoting.org/IrvRevFail.html) — Warren Smith / the Center for Range Voting, an aggressively pro-**Range** advocacy site (advocacy-adjacent: strong for mechanics, weak for verdicts). Its framing ("psychotic," "insane," "silly myth") is polemic, and the criterion it prizes — like [self-consistency](../../06_Other/RCV_IRV/concepts/RCV_IRV_non_monotonicity.md#a-sharper-critique-and-where-it-overreaches-reading-advocacy-critically) — is one *additive* methods have and elimination methods lack, so the argument is really a case for pure Score (it would fault STAR's runoff too, in principle). We include it because the library checks pro-Range anti-IRV arguments by the same standard it checks the [pro-RCV ones](../fairvote_star_whitepaper/README.md). *(The page also cites "Peru 2006" as a real instance; we haven't verified that and don't repeat it as fact.)*
