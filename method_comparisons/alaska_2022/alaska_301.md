...
**1. [Condorcet failure](../../07_Concepts/topics/condorcet/README.md).** Begich beats both rivals head-to-head (93–84 and 107–68) but isn't elected. The consensus candidate loses. *(This is the headline — see [201](alaska_201.md).)*

**2. [Spoiler effect](../../07_Concepts/topics/spoiler_effect.md).** Remove the *losing* candidate Palin from the ballots entirely, and **Begich wins** (his voters plus Palin's transfers beat Peltola). Palin could not win — but her *presence* changed who did. That is the textbook definition of a spoiler, and RCV-IRV is marketed as the method that *ends* spoilers. **Run it:** [all three candidates](cases/alaska_2022_irv_with_palin.yaml) elects Peltola 96–92; [Palin deleted from every ballot](cases/alaska_2022_irv_without_palin.yaml) elects Begich 93–84 — the same margin the pairwise grid below already reports, because with Palin gone the instant runoff *is* that head-to-head. Why this settles a specific published claim: [FairVote's comparison chart, claim-checked](../fairvote_comparison_table/README.md#3-resistance-to-spoilers-false-as-stated).

**3. [Upward monotonicity failure](../monotonicity/upward_monotonicity_alaska.md).** Take the real result (Peltola wins) and give the **winner** *more* support: had ~6,000 Palin-only voters (≈6 in the model) instead ranked **Peltola first**, Peltola would have **lost**. Why? Those votes pull Palin below Begich in Round 1, so **Palin** is eliminated instead of Begich; Begich then beats Peltola head-to-head in the final. **Ranking the winner higher made the winner lose.** A voting method should never punish a candidate for gaining support — IRV can. *(Worked on these same ballots, with its [San Francisco mirror](../monotonicity/downward_monotonicity_sf.md) alongside.)*

**4. [Majoritarian failure (ballot exhaustion)](../../06_Other/RCV_IRV/concepts/RCV_IRV_exhausted_ballots.md).** The winner's 96 votes are **48% of all 200 ballots** — a majority only of the 188 that survived after 12 exhausted. IRV's "majority winner" is a majority of *remaining* ballots, on a denominator quietly shrunk by truncated ballots.

*(The paper documents a fifth, the [no-show / truncation paradox](../../07_Concepts/topics/participation/README.md): some `Palin>Begich` voters would have gotten a better result by **not voting** at all. Same root cause — elimination order is fragile.)*

Every one of these traces to the **same** mechanism: **[eliminating candidates on first-choice counts](../../06_Other/RCV_IRV/concepts/RCV_IRV_and_plurality.md).** Methods that read the whole ballot — Ranked Robin, any Condorcet method, and STAR through its runoff — avoid all five, because none of them gate survival on first choices.
