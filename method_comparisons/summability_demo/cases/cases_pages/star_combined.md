...
[Condorcet Loser]
  No strict Condorcet loser; weak Condorcet loser: Maple (never wins a matchup) — elected by Choose-One (Plurality)!
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/star_combined_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/summability_demo/cases/star_combined.yaml
```
