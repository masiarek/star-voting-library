...
Full report from the [`_tabulated` mirror](../sntv_village_council_tabulated/sntv_village_council_tabulated.txt) (regenerated on every run; every analysis forced on):
...
```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/sntv_village_council/sntv_village_council.yaml
```
