...
Full report from the [`_tabulated` mirror](../ordered_majority_rule_tabulated/omr_opposition_decides_tabulated.txt) (regenerated on every run; every analysis forced on):
...
```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/ordered_majority_rule/omr_opposition_decides.yaml
```
