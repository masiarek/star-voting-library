...
<img src="../img/omr_opposition_decides_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 100 of 100 ballots still active; majority = 51
   C eliminated with 32:
      → B                        32

FINAL ROUND — 100 of 100 ballots still active; majority = 51
   B                        65  (65.0% of the still-active)  ← elected
   A                        35  (35.0% of the still-active)
   Never exhausted, never transferred:
      35 ballots held by A carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 100 (0.0%).
   B's 65 is a majority of the 100 still active AND of all 100 cast (65.0%).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/omr_opposition_decides_tabulated.txt) (regenerated on every run; every analysis forced on).
