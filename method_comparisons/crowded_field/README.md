...
That caveat now has a folder of its own, and it is worth reading against this one because it runs the experiment the other way round. **[Ballot expressiveness — one electorate, five papers](../ballot_expressiveness/README.md)** holds the electorate fixed and changes only what the voter may write down, so "the paper could not carry it" and "the count threw it away" become separate columns instead of one. Two results there bear directly on rung 3. Its nine-candidate score ballot is *coarser* than this one and STAR still finds the Condorcet winner — so six rungs are not automatically fatal, and rung 3 is a case rather than a law. And when its ranked ballot is capped at five names, the way New York City and Maine actually cap theirs, Ranked Robin **loses** the Condorcet winner that the 0–5 ballot kept: the full-resolution ranking assumed above is an idealization no large-field jurisdiction issues. The rates behind both — including how much of the gap at seven candidates is the paper (about 80%) versus the rule (under a point) — are in [What the ballot can and cannot say](../../07_Concepts/scores_and_ranks/ballot_expressiveness_measured.md).
