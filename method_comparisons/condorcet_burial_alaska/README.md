...
Every Condorcet method elects **Begich**. Only **RCV-IRV** elects **Peltola** — it eliminates Begich first for having the fewest first-choices ([the center squeeze](../../06_Other/RCV_IRV/concepts/RCV_IRV_center_squeeze.md); the whole point of the [Alaska case](../alaska_2022/README.md)).
...
> **Why no BetterVoting link on this one?** This case turns on the *margin-based* Condorcet methods (MinMax, Schulze, Ranked Pairs), and BetterVoting doesn't offer them — its methods are STAR / STAR-PR / Approval / Ranked Robin / IRV / Plurality / STV. On BV these ballots could only show **IRV** (→ Peltola both times — the *un*-interesting half) and **Ranked Robin / Copeland** (a 3-way **tie** in the buried cycle, which BV breaks at random). Since BetterVoting can't reproduce the point, this case is verified against [`pref_voting`](../../STARVote_LH_tabulation_engine/tools_adam/pref_voting_tabulation_engine/README.md) — which *does* implement those methods — plus the LH engine for IRV, rather than frozen from a BV run. (Same reasoning as the [RR-tiebreak case](../../05_Ranked_Robin/01_Learn/rr_tiebreak_lh_vs_bv.md): when BV can't reproduce the deterministic point, we don't mint a misleading BV backing.)
