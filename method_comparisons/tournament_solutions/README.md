...
Both cases here are the *textbook's own figures* turned back into ballots — Brandt, Brill & Harrenstein, "Tournament Solutions," ch. 3 of the [Handbook of Computational Social Choice](https://procaccia.info/wp-content/uploads/2020/03/comsoc.pdf) (2016). The chapter draws graphs; [McGarvey's theorem](../../00_start_here/topics/tournament_solutions.md) guarantees some electorate produces each one, and these three-voter profiles are the smallest we found that do. Candidate labels stay bare `A`–`E` on purpose, matching the figures, so the book can be read beside the tabulation.

Both are **LH-only**. The four-candidate case turns on a Copeland tiebreak, and BetterVoting breaks Ranked Robin ties [at random](../../00_start_here/RCV_Ranked_Robin/rr_tiebreak_lh_vs_bv.md) — an unfreezable result. The five-candidate case needs no live tally at all; its point is a disagreement between two published rules.
