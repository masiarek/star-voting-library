...
All three are **LH-only**. The `five_answers` case turns on a Copeland tiebreak, and BetterVoting breaks Ranked Robin ties [at random](../../05_Ranked_Robin/01_Learn/rr_tiebreak_lh_vs_bv.md) — an unfreezable result. The `copeland_vs_clones` case needs no live tally at all; its point is a disagreement between two published rules. The `star_elects_a_covered_candidate` case has an engine-independent STAR result, but its Ranked Robin comparison line depends on which tiebreak ladder you run.
