...
The tabulatable evidence behind [Tournament solutions — the theory of the win-loss graph](../../00_start_here/topics/tournament_solutions.md). A **tournament solution** is a rule that reads *only* the win-loss graph — who beat whom, margins thrown away — which is [Fishburn's C1 tier](../../00_start_here/topics/what_a_method_reads.md). [Ranked Robin](../../05_Ranked_Robin/concepts/ranked_robin.md) is one member of that family (it is Copeland); the academic literature holds a dozen more, and they disagree.
...
All three are **LH-only**. The `five_answers` case turns on a Copeland tiebreak, and BetterVoting breaks Ranked Robin ties [at random](../../05_Ranked_Robin/concepts/rr_tiebreak_lh_vs_bv.md) — an unfreezable result. The `copeland_vs_clones` case needs no live tally at all; its point is a disagreement between two published rules. The `star_elects_a_covered_candidate` case has an engine-independent STAR result, but its Ranked Robin comparison line depends on which tiebreak ladder you run.
...
- [The math behind Condorcet](../../05_Ranked_Robin/concepts/the_math_behind_condorcet.md) — tournaments, Smith/Schwartz · [the Smith set](../../00_start_here/topics/smith_set.md)
