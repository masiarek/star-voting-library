...
- **The textbook pair.** 4 voters raise X from second to first — X goes from **winning** to **losing**, nothing else changed. → [monotonicity worked set](../monotonicity/README.md).
- **Best = worst.** A 24-voter election where IRV elects **A** whether the voters are picking the best candidate *or* reverse every ballot to pick the worst. Its "best" and "worst" are the same guy. → [reversal symmetry](../reversal_symmetry/README.md).
...
- The failure is **IRV-specific**, not "ranked voting" — [Ranked Robin](../../05_Ranked_Robin/01_Learn/README.md) and other whole-ballot methods are monotonic.
...
There. Now you can go back to enjoying the reel — responsibly. For the version you'd actually cite in a debate, use the [worked set](../monotonicity/README.md) and [read it fairly](../paradoxes_and_whoops/reading_these_fairly.md).
