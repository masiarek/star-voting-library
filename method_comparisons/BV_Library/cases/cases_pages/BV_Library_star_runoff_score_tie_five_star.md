...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/BV_Library_star_runoff_score_tie_five_star_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/BV_Library/cases/BV_Library_star_runoff_score_tie_five_star.yaml
```
