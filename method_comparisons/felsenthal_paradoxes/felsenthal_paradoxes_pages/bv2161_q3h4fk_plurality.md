...
[Divergence from STAR]
  ...
  Note: the Approval line converts score ballots with a 3+ stars approval
        ...
        candidate has 0 approvals — the printed Approval winner (A) is just
        the candidate-priority tie-break, an artifact of the conversion, not
        an Approval verdict.
...
Everything in one file: the [`_tabulated` mirror](../felsenthal_paradoxes_tabulated/bv2161_q3h4fk_plurality_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/bv2161_q3h4fk_plurality.yaml
```
