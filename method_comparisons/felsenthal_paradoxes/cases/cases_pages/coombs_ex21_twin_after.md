...
# Coombs Ex.21 — after: two twins join for B, and B's win becomes a coin flip

*Generated from [`coombs_ex21_twin_after.yaml`](../coombs_ex21_twin_after.yaml) — do not edit by hand. Regenerate: `python STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py`.*
...
<img src="../img/coombs_ex21_twin_after_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 22 of 22 ballots still active; majority = 12
   D eliminated with 0:
      → (held no ballots)
   A eliminated with 5:
      → B                         5

FINAL ROUND — 22 of 22 ballots still active; majority = 12
   B                        13  (59.1% of the still-active)  ← elected
   C                         9  (40.9% of the still-active)
   Never exhausted, never transferred:
      9 ballots held by C carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 22 (0.0%).
   B's 13 is a majority of the 22 still active AND of all 22 cast (59.1%).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/coombs_ex21_twin_after_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/cases/coombs_ex21_twin_after.yaml
```
