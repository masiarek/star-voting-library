...
*Generated from [`coombs_ex21_twin_before.yaml`](../coombs_ex21_twin_before.yaml) — do not edit by hand. Regenerate: `python STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py`.*
...
<img src="../img/coombs_ex21_twin_before_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 20 of 20 ballots still active; majority = 11
   D eliminated with 0:
      → (held no ballots)
   A eliminated with 5:
      → B                         5

FINAL ROUND — 20 of 20 ballots still active; majority = 11
   B                        11  (55.0% of the still-active)  ← elected
   C                         9  (45.0% of the still-active)
   Never exhausted, never transferred:
      9 ballots held by C carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 20 (0.0%).
   B's 11 is a majority of the 20 still active AND of all 20 cast (55.0%).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/coombs_ex21_twin_before_tabulated.txt) (regenerated on every run; every analysis forced on).
