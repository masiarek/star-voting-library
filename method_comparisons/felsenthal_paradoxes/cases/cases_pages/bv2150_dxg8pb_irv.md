...
<img src="../img/bv2150_dxg8pb_irv_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 11 of 11 ballots still active; majority = 6
   Beth eliminated with 3:
      → Carl                      3

FINAL ROUND — 11 of 11 ballots still active; majority = 6
   Carl                      7  (63.6% of the still-active)  ← elected
   Andy                      4  (36.4% of the still-active)
   Never exhausted, never transferred:
      4 ballots held by Andy carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 11 (0.0%).
   Carl's 7 is a majority of the 11 still active AND of all 11 cast (63.6%).
```
<!-- --8<-- [end:report] -->
...
the honest answer to "who is even in contention?".
   ...
   RCV-IRV winner Carl is OUTSIDE the Smith set. ✗
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/bv2150_dxg8pb_irv_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/cases/bv2150_dxg8pb_irv.yaml
```
