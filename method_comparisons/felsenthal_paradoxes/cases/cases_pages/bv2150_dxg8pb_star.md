...
[Divergence from STAR]
  ...
  RCV-IRV rounds: cases_tabulated/bv2150_dxg8pb_star_RCV-IRV_tabulated.txt
...
[Condorcet Loser]
  Condorcet Loser: Andy — loses every head-to-head matchup — elected by Choose-One (Plurality)!
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/bv2150_dxg8pb_star_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/cases/bv2150_dxg8pb_star.yaml
```
