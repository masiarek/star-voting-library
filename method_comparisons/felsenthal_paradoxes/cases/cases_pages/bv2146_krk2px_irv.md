...
<img src="../img/bv2146_krk2px_irv_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 17 of 17 ballots still active; majority = 9
   Cleo eliminated with 4:
      → Ada                       4

FINAL ROUND — 17 of 17 ballots still active; majority = 9
   Ada                       9  (52.9% of the still-active)  ← elected
   Ben                       8  (47.1% of the still-active)
   Never exhausted, never transferred:
      8 ballots held by Ben carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 17 (0.0%).
   Ada's 9 is a majority of the 17 still active AND of all 17 cast (52.9%).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/bv2146_krk2px_irv_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/cases/bv2146_krk2px_irv.yaml
```
