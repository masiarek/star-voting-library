...
*Generated from [`succ_elim_ex12_sincere.yaml`](../succ_elim_ex12_sincere.yaml) — do not edit by hand. Regenerate: `python STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py`.*
...
Winner — Ranked Robin (RCV-RR): C
   *** 2 candidates tie on the highest Copeland score (2): C, D — tied on the tally, not a cycle (some of them beat others head-to-head, but no loop closes). Resolved by the 1st Degree tiebreaker: C has the greatest sum of win margins over the other finalists (+2).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/succ_elim_ex12_sincere_tabulated.txt) (regenerated on every run; every analysis forced on).
