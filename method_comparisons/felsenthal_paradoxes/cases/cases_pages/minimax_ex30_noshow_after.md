...
# Minimax Ex.30 — after: three C>A>B>D voters stay home, Minimax elects A

*Generated from [`minimax_ex30_noshow_after.yaml`](../minimax_ex30_noshow_after.yaml) — do not edit by hand. Regenerate: `python STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py`.*
...
Winner — Ranked Robin (RCV-RR): D
   *** 2 candidates tie for the most wins (D, B) — a Condorcet cycle (no candidate beats all others). Resolved by total margin, then lot order. (This is where Minimax / Ranked Pairs / Schulze differ — see 05_Ranked_Robin/01_Learn/cycle_resolution.md.)
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/minimax_ex30_noshow_after_tabulated.txt) (regenerated on every run; every analysis forced on).
