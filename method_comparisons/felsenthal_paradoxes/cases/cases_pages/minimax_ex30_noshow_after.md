...
Winner — Ranked Robin (RCV-RR): D
   *** 2 candidates tie for the most wins (D, B) — tied on the tally, not a cycle (some of them beat others head-to-head, but no loop closes). Resolved by the 1st Degree tiebreaker: D has the greatest sum of win margins over the other finalists (+6).
```
<!-- --8<-- [end:report] -->
