...
<img src="../img/bv2147_9gdrqg_irv_sankey.png" width="860" alt="Round-by-round Sankey diagram: each candidate's votes as a band, and where the votes of an eliminated candidate transferred to.">
...
ROUND 1 — 17 of 17 ballots still active; majority = 9
   Alma eliminated with 4:
      → Bruno                     4

FINAL ROUND — 17 of 17 ballots still active; majority = 9
   Bruno                    10  (58.8% of the still-active)  ← elected
   Cora                      7  (41.2% of the still-active)
   Never exhausted, never transferred:
      7 ballots held by Cora carried a lower ranking that was never read
      (the count stopped here, so those preferences did nothing).

Inactive ballots at the final round: 0 of 17 (0.0%).
   Bruno's 10 is a majority of the 17 still active AND of all 17 cast (58.8%).
```
<!-- --8<-- [end:report] -->
...
Everything in one file: the [`_tabulated` mirror](../cases_tabulated/bv2147_9gdrqg_irv_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/felsenthal_paradoxes/cases/bv2147_9gdrqg_irv.yaml
```
