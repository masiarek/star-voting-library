...
# The same seven voters, grouped by the FIZZY label instead — no spoiler

*Generated from [`09b_same_ballots_grouped_by_label.yaml`](../09b_same_ballots_grouped_by_label.yaml) — do not edit by hand. Regenerate: `python STARVote_LH_tabulation_engine/tools_adam/scripts/build_yaml_pages.py`.*
...
holds, spread the same way across four candidates. And the engine reports NO
...
[Vote-splitting check]
  ...
  Bloc 'Fizzy (a label, not a clone set)' = Cola, Root Beer, Diet Cola, Sparkling Water: combined 4 (57.1%); winner Diet Cola is INSIDE it.
  => No vote splitting: the bloc's own front-runner (Diet Cola) also wins
     Choose-One overall.
...
Everything in one file: the [`_tabulated` mirror](../_main_tabulated/09b_same_ballots_grouped_by_label_tabulated.txt) (regenerated on every run; every analysis forced on).

Run it yourself:

```bash
python STARVote_LH_tabulation_engine/starvote_larry_hastings.py method_comparisons/split_voting/_main/09b_same_ballots_grouped_by_label.yaml
```
