...
> **Why these are LH-only (house principle).** The teaching contrast here is **Plurality vs STAR**, not **BV vs LH** — BetterVoting and the LH engine *agree* on these results, so a BV screenshot would just duplicate the LH numbers. Two-view (BV beside LH) is reserved for cases where the two **diverge** (the bug *is* the lesson — see [Flat scores, ties & tie-breaking — and the BetterVoting bugs](../../01_STAR/09_Parked/Flat_scores_ties/), [`Runoff_07`](../../01_STAR/04_Real_Elections/runoff_reversal_bv_cases/Runoff_07_flat_ballot_bv_bug_tf73v9.md)) or where reading BV's own UI is the point.
