...
Committed cases were printing the wrong pair, including the center-squeeze
teaching case (`bv2137_ywckmg_star`, which starred Reagan though Anderson and
Carter advanced), the ice-cream tie-break ladder, and
`bv2276_qhjyr2_second_finalist_tie` — the case that exposed it.

This file covers four levels, and each catches what the others cannot:

  * the CLI, end to end — the stars and the `matrix_finalists_only` filter on
    a real rendered report;
  * the tiebreak NOTE, which explains a '*' that followed the ladder rather
    than the score;
  * the ladder itself, rung by rung, calling the resolver directly (merged
    from the parallel fix that landed on master as 4a2bedb);
  * and a sweep of every committed `_tabulated` mirror, so shipped reports
    cannot drift back.
...
# --- the ladder itself, rung by rung ----------------------------------------
#
# Everything above drives the CLI. These call the resolver directly, so a rung
# can be checked in isolation and its printed tally pinned to the exact number
# the engine shows.
...
def _load_engine():
    spec = importlib.util.spec_from_file_location("lh_engine_for_tests", WRAPPER)
