...
def case_md(r, dupes):
    ...
    M = []
    M.append(f"# {title}\n")
    # House form for a BV-backed case: the clickable /results lead line, directly
    # under the H1. On a divergence page it does real work — the reader can check
    # the disagreement below against BetterVoting's own tabulator.
    if r.get("bv"):
        test_id, election_id, results_url = r["bv"]
        tag = f" · test `{test_id}`" if test_id else ""
        M.append(f"**▶ Live on BetterVoting:** [vote](https://bettervoting.com/{election_id}) · "
                 f"**[results ↗]({results_url})** (election `{election_id}`{tag}) — "
                 "an independent tabulator to check the winners below against.\n")
...
def main():
    ...
    def _row(r, dupes):
        ...
        # Mark the BV-backed rows in the index too, so a reviewer can pick out the
        # cases that are checkable against an outside tabulator without opening each.
        bvtxt = ""
        if r.get("bv"):
            _tid, _eid, _url = r["bv"]
            bvtxt = f"  ·  [BV ↗]({_url})"
        return (f"- **[{Path(r['file']).stem}]({link})** — `{r['file']}` "
                f"({r['candidates']}c/{r['ballots']}b){bvtxt}  \n"
                f"    STAR=**{r['STAR']}** · IRV={r['IRV']} · "
                f"RR={r['RR_weak']} · Approval={r['Approval']} · "
                f"Score={r['Score']} · "
                f"Condorcet={r['Condorcet_weak'] or 'none'}{flagtxt}{duptxt}")

    n_bv = sum(1 for rs in primaries_by_bucket.values() for r, _ in rs if r.get("bv"))
    ...
    L.append(f"**{n_bv} of these {n_cases} elections are live on BetterVoting** and carry a "
             "`BV ↗` link straight to the results page — so the divergence can be checked "
             "against a tabulator nobody in this repo wrote, not just re-read from our own "
             "output. Each case page repeats the link under its title.\n")
