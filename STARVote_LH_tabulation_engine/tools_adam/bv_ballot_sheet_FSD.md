...
- **The preview's stand-in QR (preview route only).** A `--spec` preview has no election id, so it would print no QR at all — which makes the layout hard to judge. It therefore prints a **stand-in** code at full size, captioned **"TEST ONLY - sample QR"**, pointing by default at `https://bettervoting.com/qp8w68/results` — **a real but CLOSED election** (BV151), so a scan lands somewhere honest where **no vote can be cast**. That target is chosen against two worse options: an invented election id would print a scannable dead link (the failure FR-12 exists to prevent), and an *open* election would invite a stray vote into someone else's tally. `--preview-qr URL` retargets it, `--no-qr` drops it, and it is **refused on `--bv-export`** (a real ballot must carry its own election's code). Preview sheets also get a diagonal **TEST ONLY** watermark (`--no-watermark` to drop it) — unlike a header notice, it can't be cropped or overlooked.
  - *Implementation note:* `.ballot` carries `overflow: hidden` **for correctness, not looks** — the rotated `nowrap` watermark is wider than the page, and Chromium's `page.pdf()` silently scales the whole document down to fit its widest content, which shrank every ballot and undid the page-fill (FR-2a). Clipping it to the ballot box keeps the sheet at 100%.
...
![Example STAR paper ballot](../../01_STAR/concepts/img/star_paper_ballot_example.png)
...
- Teacher how-to: [`running_a_paper_ballot_demo.md`](../../01_STAR/concepts/hands_on/running_a_paper_ballot_demo.md)
- Hand-count: [`count_star_by_hand.md`](../../01_STAR/concepts/hands_on/count_star_by_hand.md) · Teaching guide: [`teaching_star_voting.md`](../../01_STAR/concepts/hands_on/teaching_star_voting.md)
