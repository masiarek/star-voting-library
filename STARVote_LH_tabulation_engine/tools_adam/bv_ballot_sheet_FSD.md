...
- **`--preview-qr [URL]` (preview route only).** A `--spec` preview has no election id, so it prints no QR — which makes the layout hard to judge. This prints a **stand-in** code at full size, captioned **"Sample QR — layout only"**, encoding the URL you pass (default `bettervoting.com`). It is **refused on `--bv-export`** (a real ballot must carry its own election's code) and it never encodes an invented election id — a fabricated id is precisely the scannable-dead-link failure FR-12 exists to prevent.
